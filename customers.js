...
// === دالة جديدة لتوحيد شكل التاريخ في العرض ===
function formatDateDisplay(dateString) {
    if (!dateString) return '-';
    let cleanDate = String(dateString).trim();
    
    // تحويل السلاش / إلى شرطة -
    if (cleanDate.includes('/')) {
        cleanDate = cleanDate.replace(/\//g, '-');
    }
    
    // إذا كان التاريخ بصيغة YYYY-MM-DD نعكسه ليصبح DD-MM-YYYY
    const parts = cleanDate.split('-');
    if (parts.length === 3) {
        if (parts[0].length === 4) { // إذا كانت السنة في البداية
...
function renderCustomers(list) {
    ...
    list.forEach((v) => {
        ...
        tr.innerHTML = `
            ...
            <!-- تم التعديل هنا لتوحيد وضبط عرض التاريخ -->
    ...
    if(monthCustomers) monthCustomers.textContent = list.filter(v => {
        ...
        if(dStr.includes('-')) {
             ...
             if(parts[0].length === 4) { // YYYY-MM-DD
                 return parseInt(parts[1])-1 === thisMonth && parseInt(parts[0]) === thisYear;
             } else { // DD-MM-YYYY
                 return parseInt(parts[1])-1 === thisMonth && parseInt(parts[2]) === thisYear;
             }
        }
...
function openAddCustomerModal() {
    ...
    // تم التعديل هنا ليكون الافتراضي بالشرطة - وليس السلاش /
    const d = new Date();
