...
// دوال مساعدة لتهيئة الوقت والتاريخ ليتوافق مع نظام الملاحظات
...
function renderCustomers(list) {
    ...
    list.forEach((v) => {
        ...
        // استخراج آخر ملاحظة لعرضها في الجدول
...
// ---------------------------------------------------------
// نظام الملاحظات الجديد المدمج مع المصفوفات والـ Firebase
// ---------------------------------------------------------
...
async function saveNote() {
    ...
    // استخدام push للإضافة في آخر المصفوفة ليتوافق مع طريقة العرض المتسلسلة
    ...
    customer.notesText = text; // احتياطي لدعم التوافق
    ...
    try {
        ...
        closeNote();
        renderCustomers(customersDataList); // إعادة تصيير الجدول لتحديث الملاحظة المختصرة
...
async function deleteNote(index) {
    ...
    if (result.isConfirmed) {
        ...
        customer.notesHistory.splice(index, 1); // إزالة الملاحظة من المصفوفة
        
        // تحديث نص الملاحظة المختصرة
        ...
        try {
            ...
            openNote(currentNoteCode); // إعادة فتح السجل لتحديث العرض الداخلي
            renderCustomers(customersDataList); // تحديث العرض الخارجي في الجدول
        } catch(error) {
...
Object.assign(window, {
    ...
    closeAddCustomerModal,
    saveNewCustomer,
    openNote,        // الدالة المحدثة
    closeNote,       // الدالة المحدثة
    saveNote,        // الدالة المحدثة
    deleteNote,      // إضافة دالة الحذف للنافذة العامة
    toggleDropdown,
    toggleAllCheckboxes,
    handleBulkAction,
    toggleLogExpansion,
