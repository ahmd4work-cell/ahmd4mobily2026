...
window.setPrimaryManager = function(selectedIndex) {
    ...
    // ربط المسؤول الرئيسي وتحديث بيانات الواجهة الرئيسية للعميل
...
window.updateManager = function(index, field, value) {
    ...
    // تحديث البيانات الأساسية تلقائياً للظهور في صفحة العملاء إذا كان هو المسؤول الرئيسي
...
window.deleteManagerRow = function(index) {
    if (confirm("هل أنت متأكد من حذف هذا المسؤول؟")) {
        ...
        if (isWasPrimary && currentCustomer.managers.length > 0) {
            ...
            // تصفير البيانات الأساسية في حال حذف كافة المسؤولين
...
window.createNewVisit = async function() {
    ...
    // 1. تجهيز كائن البيانات وفق الهيكل المعتمد في صفحة الزيارات (visits.js)
    const newVisitData = {
    ...
    try {
        // 2. إدراج الزيارة سحابياً في مجموعة الزيارات العامة (تظهر فوراً بصفحة الزيارات)
        await setDoc(doc(db, "visits", visitId), newVisitData);

        // 3. تحديث سجل زيارات العميل في الصفحة الحالية
        ...
        // التبديل إلى تبويب الزيارات لعرض السجل المحدث
