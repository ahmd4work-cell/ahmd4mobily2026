...
function updateDashboard() {
    ...
    // حساب التارجت الخاص بالفلتر المحدد المباشر
    ...
    // تحديث كارت التارجت العلوي حسب الشهر أو السنة
    ...
    // تحديث الرسوم البيانية الإجمالية بناءً على التارجت المفروز
                ...
                plugins: { 
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(ctx) {
                                if (ctx.dataIndex === 0) {
                                    return 'المكتمل: ' + (ctx.parsed || 0).toLocaleString('en-US') + ' ريال';
                                } else {
                                    return 'المتبقي للهدف: ' + (ctx.parsed || 0).toLocaleString('en-US') + ' ريال';
                                }
                            }
                        }
                    }
                ...
                plugins: { 
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(ctx) {
                                if (ctx.dataIndex === 0) {
                                    return 'المعلق: ' + (ctx.parsed || 0).toLocaleString('en-US') + ' ريال';
                                } else {
                                    return 'المتبقي للتارجت: ' + (ctx.parsed || 0).toLocaleString('en-US') + ' ريال';
                                }
                            }
                        }
                    }
...
function updateChartsLogic(salesTotal, pendingTotal, filteredSales = [], activeTarget = 180000, monthlySalesArr = [], unfilteredMonthlySalesArr = [], unfilteredMonthlyVisitsArr = []) {
    ...
    // حساب النسب المئوية بناءً على التارجت المفروز الحالي
    ...
    // تحديث قيم النسب في منتصف الدوائر
    ...
    // تحديث المبالغ الموضحة أسفل الدوائر (المحيط / التارجت)
