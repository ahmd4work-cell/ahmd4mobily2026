...
let monthlyCompletedChart, monthlyPendingChart; // المتغيرات الجديدة الخاصة بالدائرتين تحت الجدول
...
// دالة ذكية لتوليد تدرج ألوان للخريطة الحرارية (ألوان الدوائر الجديدة)
function generateHeatmapColors(values, baseColorRGB) {
    const max = Math.max(...values);
    if (max === 0) return Array(12).fill('#f1f5f9'); // في حال كانت كل الشهور صفر
    return values.map(v => {
        if (v === 0) return '#f8fafc'; // لون باهت جداً للصفر
        const opacity = 0.3 + (0.7 * (v / max)); // تدرج شفافية بناءً على حجم المبيعات لتمييز الشهر الأفضل
...
function updateDashboard() {
    ...
    let monthlyPendingArr = Array(12).fill(0); // المصفوفة الجديدة للمعلقات الشهرية
    ...
    filteredSales.forEach(sale => {
        ...
        if (month >= 1 && month <= 12) {
            ...
            monthlyPendingArr[month - 1] += sale.pendingSum; // تسجيل المعلق الشهري
        }
    });
    ...
    // إرسال المصفوفات الجديدة للرسومات البيانية
    updateChartsLogic(totalSales, totalPending, filteredSales, yearlyTargetSum, monthlySalesArr, monthlyPendingArr);
...
function initCharts() {
    ...
    // إعداد دوائر التوزيع الشهري (تحت الجدول)
    // نجعل الداتا ثابتة بـ 1 لعدد 12 جزء للحصول على 12 شريحة متساوية تعبر عن الشهور
    const distChartOptions = {
        ...
        plugins: {
            legend: { display: false },
            tooltip: { callbacks: { label: function(ctx) { return monthsNames[ctx.dataIndex] + ': 0 ريال'; } } }
        }
    };
    ...
    const mpCtx = document.getElementById('monthlyPendingChart');
    if (mpCtx) {
        monthlyPendingChart = new Chart(mpCtx, {
            type: 'doughnut',
            data: { labels: monthsNames, datasets: [{ data: Array(12).fill(1), backgroundColor: Array(12).fill('#e2e8f0'), borderWidth: 2, borderColor: '#ffffff' }] },
            options: JSON.parse(JSON.stringify(distChartOptions))
        });
...
// دالة التحديث الشاملة (تستقبل المصفوفات الجديدة)
function updateChartsLogic(salesTotal, pendingTotal, filteredSales = [], yearlyTargetSum = 180000, monthlySalesArr = [], monthlyPendingArr = []) {
    ...
    // تحديث الدائرتين الخاصتين بالتحليل الشهري تحت الجدول
    if (monthlyCompletedChart && monthlySalesArr.length > 0) {
        monthlyCompletedChart.data.datasets[0].backgroundColor = generateHeatmapColors(monthlySalesArr, '16, 185, 129'); // لون أخضر
    ...
    if (monthlyPendingChart && monthlyPendingArr.length > 0) {
        monthlyPendingChart.data.datasets[0].backgroundColor = generateHeatmapColors(monthlyPendingArr, '245, 158, 11'); // لون برتقالي
        monthlyPendingChart.options.plugins.tooltip.callbacks.label = function(ctx) {
            return monthsNames[ctx.dataIndex] + ': ' + (monthlyPendingArr[ctx.dataIndex] || 0).toLocaleString('en-US') + ' ريال';
        };
        monthlyPendingChart.update();
    }
