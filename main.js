...
// جلب وحفظ أهداف الأشهر سحابياً وفي LocalStorage كنسخة إضافية
...
async function saveStoredTargets(targetsArr) {
    ...
    try {
        await setDoc(doc(db, "settings", "monthlyTargets"), { values: targetsArr }, { merge: true });
    } catch (e) {
...
function listenToFirestoreData() {
    ...
    // الاستماع لأهداف المبيعات الشهرية سحابياً
