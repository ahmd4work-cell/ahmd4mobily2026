...
function listenToFirestoreData() {
    ...
    const checkAndUpdate = () => {
        ...
        salesDb.forEach(order => {
            ...
            linkedSales.push({
                id: order.id, date: order.date || order.createdAt || '', region: order.region || '',
                supervisor: order.supervisor || '', salesman: order.owner || order.salesman || '',
                completedSum: completedSum, pendingSum: pendingSum
            });
        });
...
function fillSelect(selectElement, setOrArray, defaultVal, isMonth = false) {
    ...
    defaultOpt.text = defaultVal; defaultOpt.value = defaultVal === "الكل" ? "all" : defaultVal;
    selectElement.appendChild(defaultOpt);

    setOrArray.forEach((val, index) => {
        if(isMonth && val === defaultVal) return;
        const opt = document.createElement('option');
        opt.text = val; opt.value = isMonth ? (index + 1).toString().padStart(2, '0') : val; 
...
function updateDashboard() {
    ...
    // ==========================================
    // التعديل المطلوب: فلترة الفرص البيعية لتكون نشطة فقط (مهتم أو تأهيل لفرصة) 
    // لتتطابق تماماً مع مربع الإحصائيات في صفحة الفرص البيعية
    // ==========================================
    const activeOpps = filteredOpps.filter(o => o.status === 'مهتم' || o.status === 'تأهيل لفرصة');
    ...
    // تحديث كروت الإحصائيات بالبيانات الدقيقة
    if(oppCountEl) oppCountEl.innerText = activeOpps.length.toLocaleString('en-US'); 
    ...
    // إرسال activeOpps بدلاً من كل الفرص لضمان دقة أرقام الجدول
    const yearlyTargetSum = updateYearlyTable(filteredSales, activeOpps, filteredVisits);
