// ==========================================
// main.js - لوحة القيادة العامة وتكامل السحابة والوضع الليلي
// ==========================================
...
function initDarkMode() {
    ...
    darkModeToggle.addEventListener('click', () => {
        ...
        updateDashboard(); // لتحديث الرسوم والألوان الحيوية
    });
}
...
function updateChartsTheme(isDark) {
    ...
    if (staffChart) {
        if (staffChart.options.scales.x) {
            staffChart.options.scales.x.ticks.color = isDark ? '#f8fafc' : '#334155';
        }
...
function parseDateParts(dateStr) {
    ...
    if (parts.length === 3) {
        ...
        if (p0.length === 4) {
            return { day: p2.padStart(2, '0'), month: p1, year: p0 };
        }
        if (p2.length === 4) {
...
function parseMonthFromDate(dateStr) {
    return parseDateParts(dateStr).month;
}
...
// دالة ذكية لتوليد تدرج ألوان للخريطة الحرارية (مع دعم الوضع الليلي)
...
// جلب وحفظ أهداف الأشهر محلياً وسحابياً
...
function updateDashboard() {
    ...
    const tableSales = data.sales.filter(tableFilterCallback);
    const tableOpps = data.opportunities.filter(tableFilterCallback);
    const tableVisits = data.visits.filter(tableFilterCallback);
    ...
    let unfilteredMonthlySalesArr = Array(12).fill(0);
    let unfilteredMonthlyVisitsArr = Array(12).fill(0);

    tableSales.forEach(sale => {
        let month = parseInt(parseMonthFromDate(sale.date));
        if (month >= 1 && month <= 12) {
            unfilteredMonthlySalesArr[month - 1] += sale.completedSum;
        }
    });
    ...
    tableVisits.forEach(v => {
        ...
        if (month >= 1 && month <= 12) {
            unfilteredMonthlyVisitsArr[month - 1] += 1;
        }
    });
...
function updateYearlyTable(sales, opps, visits) {
    ...
    const totTargetEl = document.getElementById('totTarget'), totCompletedEl = document.getElementById('totCompleted'), totPendingEl = document.getElementById('totPending'), totVisitsEl = document.getElementById('totVisits'), totOppsEl = document.getElementById('totOpps'), cardTargetEl = document.getElementById('yearlyTargetCardVal');
...
function updateChartsLogic(salesTotal, pendingTotal, filteredSales = [], yearlyTargetSum = 180000, monthlySalesArr = [], unfilteredMonthlySalesArr = [], unfilteredMonthlyVisitsArr = []) {
    ...
    if (monthlyCompletedChart && unfilteredMonthlySalesArr.length > 0) {
        ...
        monthlyCompletedChart.options.plugins.tooltip.callbacks.label = function(ctx) {
            ...
            return [ `${monthsNames[ctx.dataIndex]}: ${val.toLocaleString('en-US')} ريال`, `النسبة من الهدف السنوي: ${pct}%` ];
        };
    ...
    if (monthlyVisitsChart && unfilteredMonthlyVisitsArr.length > 0) {
        ...
        monthlyVisitsChart.options.plugins.tooltip.callbacks.label = function(ctx) {
            ...
            const pct = totalVisitsVolume > 0 ? ((val / totalVisitsVolume) * 100).toFixed(1) : 0;
            return [ `${monthsNames[ctx.dataIndex]}: ${val.toLocaleString('en-US')} زيارة`, `النسبة من إجمالي الزيارات: ${pct}%` ];
        };
