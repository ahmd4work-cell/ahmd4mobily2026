...
let monthlyCompletedChart, monthlyVisitsChart; // تم تغيير معلق إلى الزيارات
...
function updateDashboard() {
    ...
    let monthlyVisitsArr = Array(12).fill(0); // المصفوفة الخاصة بعدد الزيارات
    ...
    // استخراج أعداد الزيارات الشهرية
    filteredVisits.forEach(v => {
        let month = parseInt(parseMonthFromDate(v.date || v.visitDate || v.oppDate));
        if (month >= 1 && month <= 12) {
    ...
    // تمرير البيانات للدوال البيانية شاملة مصفوفة الزيارات
...
function initCharts() {
    ...
    const distChartOptions = {
        cutout: '72%', // توسيع الفجوة الداخلية لاستيعاب الكلمة
        responsive: true,
        maintainAspectRatio: false,
        plugins: {
            legend: { display: false },
...
// دالة التحديث الشاملة مع اعتماد أحجام الدوائر على نسبة القيم
function updateChartsLogic(salesTotal, pendingTotal, filteredSales = [], yearlyTargetSum = 180000, monthlySalesArr = [], monthlyVisitsArr = []) {
    ...
    // تحديث دائرة "مكتمل" لتكون مبنية على نسبة المبيعات وتوزيع الشهور
    if (monthlyCompletedChart && monthlySalesArr.length > 0) {
        const totalSalesVolume = monthlySalesArr.reduce((a, b) => a + b, 0);
        // جعل حجم الشريحة يعتمد على حجم مبيعات الشهر (إذا كان الإجمالي 0، نضع مقاسات متساوية)
    ...
    // تحديث دائرة "الزيارات" لتكون مبنية على نسبة الزيارات الفعلية
    if (monthlyVisitsChart && monthlyVisitsArr.length > 0) {
        const totalVisitsVolume = monthlyVisitsArr.reduce((a, b) => a + b, 0);
        // جعل حجم الشريحة يعتمد على عدد الزيارات في الشهر بناءً على الإجمالي
        monthlyVisitsChart.data.datasets[0].data = totalVisitsVolume === 0 ? Array(12).fill(1) : monthlyVisitsArr;
        monthlyVisitsChart.data.datasets[0].backgroundColor = generateHeatmapColors(monthlyVisitsArr, '71, 85, 105'); // لون رمادي مزرق يناسب الزيارات
