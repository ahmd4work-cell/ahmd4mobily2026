...
// دالة معالجة التواريخ المعززة للربط الدقيق مع صفحة الزيارات
...
function updateDashboard() {
    ...
    // 1. الفرز العام لأغلب عناصر اللوحة (الكروت، المؤشرات، الأرباع، أداء الموظفين)
    ...
    // 2. الفرز الخاص بجدول السجل والدائرتين السفلية (يعتمد على السنة فقط ولا يتأثر بالفرع/المشرف/المندوب/الشهر)
    ...
    // حساب البيانات المفروزة لباقي الصفحة
    ...
    // حساب البيانات الشاملة للجدول والدائرتين تحت الجدول
    ...
    // تحديث الجدول بالبيانات الكلية (ثابتة لا تتغير مع فرز الفروع والمشرفين والمندوبين)
    const yearlyTargetSum = updateYearlyTable(tableSales, tableOpps, tableVisits);
    
    // تحديث الرسوم البيانية مع تمرير المصفوفات الكلية للدائرتين
    ...
    const distChartOptions = {
        ...
        plugins: {
            ...
            tooltip: {
                ...
                cornerRadius: 6,
                displayColors: false,
                callbacks: { label: function(ctx) { return monthsNames[ctx.dataIndex] + ': 0'; } }
            }
        }
    };
...
function updateChartsLogic(salesTotal, pendingTotal, filteredSales = [], yearlyTargetSum = 180000, monthlySalesArr = [], unfilteredMonthlySalesArr = [], unfilteredMonthlyVisitsArr = []) {
    ...
    // === تحديث دائرة المكتمل بالبيانات الشاملة (غير المفروزة) ===
    if (monthlyCompletedChart && unfilteredMonthlySalesArr.length > 0) {
        ...
        monthlyCompletedChart.options.plugins.tooltip.callbacks.label = function(ctx) {
            ...
            return [
                `${monthsNames[ctx.dataIndex]}: ${val.toLocaleString('en-US')} ريال`,
                `النسبة من الهدف السنوي: ${pct}%`
            ];
        };
        monthlyCompletedChart.update();
    }
    
    // === تحديث دائرة الزيارات بالبيانات الشاملة (غير المفروزة) ===
    if (monthlyVisitsChart && unfilteredMonthlyVisitsArr.length > 0) {
        ...
        monthlyVisitsChart.data.datasets[0].backgroundColor = generateHeatmapColors(unfilteredMonthlyVisitsArr, '71, 85, 105');
        ...
        monthlyVisitsChart.options.plugins.tooltip.callbacks.label = function(ctx) {
            ...
            return [
                `${monthsNames[ctx.dataIndex]}: ${val.toLocaleString('en-US')} زيارة`,
                `النسبة من إجمالي الزيارات: ${pct}%`
            ];
        };
