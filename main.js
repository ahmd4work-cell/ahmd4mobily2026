...
let completedMonthsChart, pendingMonthsChart; // الرسوم البيانية الجديدة للأشهر
...
function updateDashboard() {
    ...
    // تحديث الرسومات البيانية الخاصة بالأشهر 
...
function initCharts() {
    ...
    // --- إعداد الرسومات البيانية الخاصة بالأشهر 12 ---
    const months12Colors = [
    ...
    const monthlyPieOptions = {
        ...
        maintainAspectRatio: false,
        plugins: {
            legend: { display: false }, // إخفاء المفاتيح للحفاظ على المساحة واستخدام الـ Tooltip
            tooltip: {
                callbacks: {
                    label: function(ctx) {
    ...
    const ctxComp = document.getElementById('completedMonthsChart');
    if (ctxComp) {
        completedMonthsChart = new Chart(ctxComp, {
            type: 'doughnut',
            data: { 
                labels: monthsNames, 
                datasets: [{ data: Array(12).fill(0), backgroundColor: months12Colors, borderWidth: 1 }] 
            },
            options: monthlyPieOptions
        });
    }

    const ctxPend = document.getElementById('pendingMonthsChart');
    if (ctxPend) {
        pendingMonthsChart = new Chart(ctxPend, {
            type: 'doughnut',
            data: { 
                labels: monthsNames, 
                datasets: [{ data: Array(12).fill(0), backgroundColor: months12Colors, borderWidth: 1 }] 
