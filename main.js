// ==========================================
// main.js - لوحة القيادة العامة وتكامل السحابة والوضع الليلي (مع نظام البحث المخصص)
// ==========================================
import { db } from './firebase-config.js';
import { collection, onSnapshot, doc, setDoc } from "https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore.js";

// إضافة customers لتخزين بيانات العملاء للبحث
...
function parseDateParts(dateStr) {
    if (!dateStr || typeof dateStr !== 'string') return { day: null, month: null, year: null };
    ...
    if (parts.length === 3) {
        ...
        let p2 = parts[2];
        if (p0.length === 4) return { day: p2.padStart(2, '0'), month: p1, year: p0 };
        if (p2.length === 4) return { day: p0, month: p1, year: p2 };
    }
...
function initTargetEditToggle() {
    ...
    editBtn.addEventListener('click', () => {
        ...
        if (!isEditingTargets) {
            ...
            if (inputs.length === 12) {
                saveStoredTargets(Array.from(inputs).map(inp => parseFloat(inp.value) || 0));
            }
        }
        updateDashboard();
    });
...
function listenToFirestoreData() {
    ...
    onSnapshot(collection(db, "customers"), (snapshot) => { customersData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })); checkAndUpdate(); });
...
function populateFilterOptions() {
    ...
    allData.forEach(item => {
        ...
        let itemDate = item.visitDate || item.date || item.oppDate || item.saleDate;
        if (itemDate) {
...
function updateDashboard() {
    ...
    const filterCallback = (item) => {
        const iDate = item.visitDate || item.date || item.oppDate || item.saleDate || "";
    ...
    filteredSales.forEach(sale => {
        totalSales += sale.completedSum; totalPending += sale.pendingSum;
        let month = parseInt(parseMonthFromDate(sale.date));
        if (month >= 1 && month <= 12) monthlySalesArr[month - 1] += sale.completedSum;
    });

    tableSales.forEach(sale => {
        let month = parseInt(parseMonthFromDate(sale.date));
        if (month >= 1 && month <= 12) unfilteredMonthlySalesArr[month - 1] += sale.completedSum;
    });
    ...
    updateChartsLogic(totalSales, totalPending, filteredSales, yearlyTargetSum, monthlySalesArr, unfilteredMonthlySalesArr, unfilteredMonthlyVisitsArr);
    ...
    if (isEditingTargets) {
        document.querySelectorAll('.target-input').forEach(inp => {
            inp.addEventListener('input', (e) => {
                ...
                targets[idx] = val; saveStoredTargets(targets);
                ...
                if (document.getElementById('yearlyTargetCardVal')) document.getElementById('yearlyTargetCardVal').innerText = newTotal.toLocaleString('en-US');
            });
        });
    }

    if (document.getElementById('totTarget')) document.getElementById('totTarget').innerText = totalTarget.toLocaleString('en-US');
    if (document.getElementById('yearlyTargetCardVal')) document.getElementById('yearlyTargetCardVal').innerText = totalTarget.toLocaleString('en-US');
...
function initCharts() {
    ...
    if (achievedEl) achievedChart = new Chart(achievedEl, { type: 'doughnut', data: { datasets: [{ data: [0, 100], backgroundColor: ['#10b981', emptyBg], borderWidth: 0, borderRadius: 10 }] }, options: { cutout: '82%', responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } } });

    const pendingEl = document.getElementById('pendingChart');
    if (pendingEl) pendingChart = new Chart(pendingEl, { type: 'doughnut', data: { datasets: [{ data: [0, 100], backgroundColor: ['#f59e0b', emptyBg], borderWidth: 0, borderRadius: 10 }] }, options: { cutout: '82%', responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } } });
    ...
    const qInitialData = { datasets: [{ data: [0, 0, 0, 100], backgroundColor: [emptyBg, emptyBg, emptyBg, emptyBg], borderWidth: 2, borderColor: borderColor }] };
    ...
    if (mcCtx) monthlyCompletedChart = new Chart(mcCtx, { type: 'doughnut', data: { labels: monthsNames, datasets: [{ data: Array(12).fill(1), backgroundColor: Array(12).fill(emptyBg), borderWidth: 2, borderColor: borderColor }] }, options: JSON.parse(JSON.stringify(distChartOptions)) });
    const mvCtx = document.getElementById('monthlyVisitsChart');
    if (mvCtx) monthlyVisitsChart = new Chart(mvCtx, { type: 'doughnut', data: { labels: monthsNames, datasets: [{ data: Array(12).fill(1), backgroundColor: Array(12).fill(emptyBg), borderWidth: 2, borderColor: borderColor }] }, options: JSON.parse(JSON.stringify(distChartOptions)) });
...
function updateChartsLogic(salesTotal, pendingTotal, filteredSales = [], yearlyTargetSum = 180000, monthlySalesArr = [], unfilteredMonthlySalesArr = [], unfilteredMonthlyVisitsArr = []) {
    const isDark = document.body.classList.contains('dark-mode');
    const emptyBg = isDark ? '#1e293b' : '#f1f5f9';
    ...
    const gaugePercentRaw = yearlyTargetSum > 0 ? (salesTotal / yearlyTargetSum) * 100 : 0;
    ...
    if (achievedChart) { achievedChart.data.datasets[0].data = [salesPercent, 100 - salesPercent]; achievedChart.data.datasets[0].backgroundColor = ['#10b981', emptyBg]; achievedChart.update(); }
    if (pendingChart) { pendingChart.data.datasets[0].data = [pendingPercent, 100 - pendingPercent]; pendingChart.data.datasets[0].backgroundColor = ['#f59e0b', emptyBg]; pendingChart.update(); }
    ...
    if (monthlyCompletedChart && unfilteredMonthlySalesArr.length > 0) {
        ...
        monthlyCompletedChart.options.plugins.tooltip.callbacks.label = function(ctx) {
            ...
            const target = yearlyTargetSum || 180000;
            return [ `${monthsNames[ctx.dataIndex]}: ${val.toLocaleString('en-US')} ريال`, `النسبة من الهدف السنوي: ${target > 0 ? ((val / target) * 100).toFixed(1) : 0}%` ];
        };
    ...
    if (staffChart) {
        ...
        staffChart.data.labels = Array.from({length: 30}, (_, i) => realStaffNames[i] || `موظف ${i + 1}`);
        staffChart.data.datasets[0].data = Array.from({length: 30}, (_, i) => realStaffNames[i] ? staffAggregation[realStaffNames[i]].completed : 0);
        staffChart.data.datasets[1].data = Array.from({length: 30}, (_, i) => realStaffNames[i] ? staffAggregation[realStaffNames[i]].pending : 0);
        staffChart.update();
    }
}

// ==========================================
// نظام البحث الشامل الدقيق
// ==========================================
function initGlobalSearch() {
    ...
    searchInput.addEventListener('input', (e) => {
        ...
        // جلب البيانات بما فيها العملاء
        const { visits, opportunities, sales, customers } = firestoreRawData;
        let results = [];

        // معالجة البحث في الأعمدة المطلوبة فقط (الشركة / المسؤول / رقم التواصل / الإيميل / السجل)
        const processItems = (arr, pageName, pageUrl) => {
            arr.forEach(item => {
                // تجميع الحقول المحتملة للأعمدة المستهدفة باختلاف مسمياتها في الداتا بيز
                const targetFields = [
                    // الشركة
                    item.companyName, item.clientName, item.customerName, item.customer, item.company,
                    // المسؤول
                    item.manager, item.responsible, item.contactPerson, item.contactName, item.personInCharge,
                    // رقم التواصل
                    item.phone, item.mobile, item.contact, item.contactNumber, item.telephone,
                    // الإيميل
                    item.email, item.mail,
                    // السجل (أو المعرف)
                    item.cr, item.crNumber, item.commercialRecord, item.record, item.id
                ];

                // البحث الدقيق داخل الحقول المحددة فقط
                const matches = targetFields.some(val => 
                    val && String(val).toLowerCase().includes(query)
                );
                
                if (matches) {
                    const displayName = item.companyName || item.clientName || item.customerName || item.customer || item.name || item.id || 'سجل بدون اسم';
                    // تجنب تكرار نفس السجل في نفس الصفحة
                    if (!results.find(r => r.id === item.id && r.pageName === pageName)) {
                        results.push({ pageName, pageUrl, displayName, id: item.id });
                    }
                }
            });
        };

        // البحث في الصفحات الأربعة المحددة
        processItems(sales, 'المبيعات', 'sales.html');
        processItems(opportunities, 'الفرص البيعية', 'opportunities.html');
        processItems(customers, 'العملاء', 'customers.html');
        processItems(visits, 'الزيارات', 'visits.html');

        searchResults.innerHTML = '';
        if (results.length > 0) {
            results.slice(0, 20).forEach(res => {
                const a = document.createElement('a');
                a.href = `${res.pageUrl}?id=${res.id}`;
                a.className = 'search-item';
                a.innerHTML = `
                    <span class="search-item-page">${res.pageName}</span>
                    <span class="search-item-details">${res.displayName}</span>
                `;
                searchResults.appendChild(a);
            });
        } else {
            searchResults.innerHTML = `<div class="search-item"><span class="search-item-details">لا توجد نتائج مطابقة...</span></div>`;
        }
