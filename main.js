// ==========================================
// main.js - لوحة القيادة العامة وتكامل السحابة (محدث ومحسن نهائياً)
// ==========================================
...
function populateFilterOptions() {
    ...
    allData.forEach(item => {
        ...
        let itemDate = item.date || item.visitDate || item.visit_date || item.createdAt || item.created_at || item.timestamp || item.oppDate || item.saleDate;
        if (itemDate) {
...
function updateDashboard() {
    ...
    const filterCallback = (item) => {
        const iDate = item.date || item.visitDate || item.visit_date || item.createdAt || item.created_at || item.timestamp || item.oppDate || item.saleDate || "";
    ...
    filteredVisits.forEach(v => {
        let vDate = v.date || v.visitDate || v.visit_date || v.createdAt || v.created_at || v.timestamp || v.oppDate;
        let month = parseInt(parseMonthFromDate(vDate));
        if (month >= 1 && month <= 12) {
...
function updateYearlyTable(sales, opps, visits) {
    ...
    monthsNames.forEach((monthName, index) => {
        ...
        const mOpps = opps.filter(o => {
            const oDate = o.date || o.oppDate || o.visitDate || o.visit_date || o.createdAt || o.created_at || o.timestamp;
            return parseMonthFromDate(oDate) === monthCode;
        });

        const mVisits = visits.filter(v => {
            const vDate = v.date || v.visitDate || v.visit_date || v.createdAt || v.created_at || v.timestamp || v.oppDate;
            return parseMonthFromDate(vDate) === monthCode;
        });
