...
let monthlyCompletedChart, monthlyPendingChart; // المتغيرات الخاصة بالدائرتين الجديدتين
...
// ألوان متناسقة للـ 12 شهر
const monthColors = ['#10b981', '#3b82f6', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899', '#14b8a6', '#84cc16', '#6366f1', '#f97316', '#06b6d4', '#eab308'];
...
function updateDashboard() {
    ...
    if (qTitle) qTitle.innerText = `تحليل مبيعات الأرباع السنوية (هدف الربع: ${Math.round(avgQuarterTarget/1000)}k)`;
...
function initCharts() {
    ...
    // الدوائر الخاصة بالأرباع السنوية
    ...
    // تهيئة دائرتي المكتمل والمعلق شهرياً الجديدتين
    const breakdownOptions = {
        ...
        plugins: {
            ...
            tooltip: {
                callbacks: {
                    label: function(ctx) {
                        return ctx.label + ': ' + (ctx.parsed || 0).toLocaleString('en-US') + ' ريال';
                    }
                }
            }
        }
    ...
    const monthlyCompEl = document.getElementById('monthlyCompletedChart');
    if (monthlyCompEl) {
        monthlyCompletedChart = new Chart(monthlyCompEl, {
            type: 'doughnut',
            data: { labels: monthsNames, datasets: [{ data: Array(12).fill(0), backgroundColor: monthColors, borderWidth: 1 }] },
            options: breakdownOptions
        });
    }

    const monthlyPendEl = document.getElementById('monthlyPendingChart');
    if (monthlyPendEl) {
        monthlyPendingChart = new Chart(monthlyPendEl, {
            type: 'doughnut',
            data: { labels: monthsNames, datasets: [{ data: Array(12).fill(0), backgroundColor: monthColors, borderWidth: 1 }] },
            options: breakdownOptions
        });
...
function updateChartsLogic(salesTotal, pendingTotal, filteredSales = [], yearlyTargetSum = 180000, monthlySalesArr = [], monthlyPendingArr = []) {
    ...
    // تحديث الدائرتين الجديدتين الخاصتين بالشهور
    if (monthlyCompletedChart && monthlySalesArr.length === 12) {
        monthlyCompletedChart.data.datasets[0].data = monthlySalesArr;
        monthlyCompletedChart.update();
    }
    if (monthlyPendingChart && monthlyPendingArr.length === 12) {
        monthlyPendingChart.data.datasets[0].data = monthlyPendingArr;
