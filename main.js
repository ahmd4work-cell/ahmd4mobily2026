...
// دالة معالجة التواريخ المعززة للربط الدقيق مع صفحة الزيارات (تتدعم DD-MM-YYYY و YYYY-MM-DD)
function parseDateParts(dateStr) {
    ...
    if (parts.length === 3) {
        ...
        // النمط: DD-MM-YYYY (المستخدم في صفحة الزيارات)
        if (p2.length === 4) {
...
function updateChartsLogic(salesTotal, pendingTotal, filteredSales = [], yearlyTargetSum = 180000, monthlySalesArr = [], monthlyVisitsArr = []) {
    ...
    if (monthlyVisitsChart && monthlyVisitsArr.length > 0) {
        ...
        monthlyVisitsChart.options.plugins.tooltip.callbacks.label = function(ctx) {
            return monthsNames[ctx.dataIndex] + ': ' + (monthlySalesArr[ctx.dataIndex] || 0).toLocaleString('en-US') + ' زيارة';
        };
