// ==========================================
// main.js - لوحة القيادة العامة وتكامل السحابة
// ==========================================
...
let completedMonthsChart, pendingMonthsChart; // الدوائر الجديدة
...
// دالة مساعدة لتحديث حالة المزامنة في واجهة المستخدم
...
// دالة تفعيل الضبابية مع الحفظ الدائم في LocalStorage
...
// دالة تعديل الأهداف عند الضغط على القلم
...
// الاستماع اللحظي لكافة بيانات السحابة (Firestore)
function listenToFirestoreData() {
    let visitsData = [], oppsData = [], salesDb = [], productsDb = {};
    updateSyncStatus('loading');

    // الاستماع لأهداف المبيعات الشهرية سحابياً ومزامنتها محلياً
...
function updateDashboard() {
    ...
    const filterCallback = (item) => {
        const iDate = item.date || item.visitDate || item.oppDate || item.saleDate || "";
    ...
    let monthlyPendingArr = Array(12).fill(0);
    ...
    filteredSales.forEach(sale => {
        ...
        if (month >= 1 && month <= 12) {
            ...
            monthlyPendingArr[month - 1] += sale.pendingSum;
        }
    });
    ...
    updateChartsLogic(totalSales, totalPending, filteredSales, yearlyTargetSum);
    ...
    // تحديث الدوائر الجديدة الخاصة بتحليل الأشهر
    if(completedMonthsChart) {
        const totalC = monthlySalesArr.reduce((a,b)=>a+b,0);
        if(totalC === 0) {
            completedMonthsChart.data.datasets[0].data = Array(12).fill(1); // إظهار شكل دائرة فارغة إذا لم تكن هناك بيانات
            completedMonthsChart.data.datasets[0].backgroundColor = Array(12).fill('#e2e8f0');
        } else {
            completedMonthsChart.data.datasets[0].data = monthlySalesArr;
            completedMonthsChart.data.datasets[0].backgroundColor = ['#022c22', '#064e3b', '#065f46', '#047857', '#059669', '#10b981', '#34d399', '#6ee7b7', '#a7f3d0', '#0ea5e9', '#0284c7', '#0369a1'];
        }
        completedMonthsChart.update();
    }

    if(pendingMonthsChart) {
        const totalP = monthlyPendingArr.reduce((a,b)=>a+b,0);
        if(totalP === 0) {
            pendingMonthsChart.data.datasets[0].data = Array(12).fill(1);
            pendingMonthsChart.data.datasets[0].backgroundColor = Array(12).fill('#e2e8f0');
        } else {
            pendingMonthsChart.data.datasets[0].data = monthlyPendingArr;
            pendingMonthsChart.data.datasets[0].backgroundColor = ['#451a03', '#7c2d12', '#9a3412', '#c2410c', '#ea580c', '#f97316', '#fb923c', '#fdba74', '#f59e0b', '#d97706', '#b45309', '#78350f'];
        }
        pendingMonthsChart.update();
    }
}
...
function updateYearlyTable(sales, opps, visits) {
    ...
    monthsNames.forEach((monthName, index) => {
        ...
        const mOpps = opps.filter(o => parseMonthFromDate(o.date || o.oppDate || o.visitDate) === monthCode);
        const mVisits = visits.filter(v => parseMonthFromDate(v.date || v.visitDate || v.oppDate) === monthCode);
...
function initCharts() {
    ...
    // تهيئة الدوائر الجديدة للأشهر (مكتمل ومعلق)
    const monthsTooltipOptions = {
        callbacks: {
            label: function(ctx) {
                let label = monthsNames[ctx.dataIndex] || '';
                if (label) label += ': ';
                if (ctx.parsed !== null) label += Number(ctx.parsed).toLocaleString('en-US') + ' ريال';
                return label;
            }
        }
    };

    const cMonthsEl = document.getElementById('completedMonthsChart');
    if (cMonthsEl) {
        completedMonthsChart = new Chart(cMonthsEl, {
            type: 'doughnut',
            data: {
                labels: monthsNames,
                datasets: [{ data: Array(12).fill(1), backgroundColor: Array(12).fill('#e2e8f0'), borderWidth: 1, borderColor: '#fff' }]
            },
            options: { cutout: '55%', responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: monthsTooltipOptions } }
        });
    }

    const pMonthsEl = document.getElementById('pendingMonthsChart');
    if (pMonthsEl) {
        pendingMonthsChart = new Chart(pMonthsEl, {
            type: 'doughnut',
            data: {
                labels: monthsNames,
                datasets: [{ data: Array(12).fill(1), backgroundColor: Array(12).fill('#e2e8f0'), borderWidth: 1, borderColor: '#fff' }]
...
function updateChartsLogic(salesTotal, pendingTotal, filteredSales = [], yearlyTargetSum = 180000) {
