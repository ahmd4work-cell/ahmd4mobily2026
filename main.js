// ==========================================
// main.js - لوحة القيادة العامة وتكامل السحابة والوضع الليلي المطور
// ==========================================
...
// دالة تأخير التنفيذ لتوفير طلبات السحابة (Debounce)
...
// ==========================================
// دالة تحليل التواريخ المتطورة وشاملة لكافة الأشكال
// ==========================================
...
// حفظ محلي مع الحفظ المؤجل للفايربيس
...
function initCharts() {
    ...
    // الرسم البياني الديناميكي لفريق المبيعات
...
function updateChartsLogic(salesTotal, pendingTotal, filteredSales = [], yearlyTargetSum = 180000, monthlySalesArr = [], unfilteredMonthlySalesArr = [], unfilteredMonthlyVisitsArr = []) {
    ...
    if (staffChart) {
        ...
        // ترتيب الموظفين تنازلياً حسب الإنجاز
...
// ==========================================
// نظام البحث الشامل المطور بدعم الكيبورد وتأخير الإدخال
// ==========================================
function initGlobalSearch() {
    ...
    const executeSearch = debounce((query) => {
        ...
        if (results.length > 0) {
            results.slice(0, 15).forEach((res, i) => {
                ...
                // إبراز الكلمة المبحوث عنها
                const regExp = new RegExp(`(${query})`, 'gi');
                const highlightedName = res.displayName.replace(regExp, '<mark>$1</mark>');

                a.innerHTML = `
                    <span class="search-item-page">${res.pageName}</span>
                    <span class="search-item-details">${highlightedName}</span>
                `;
    ...
    searchInput.addEventListener('input', (e) => executeSearch(e.target.value.trim().toLowerCase()));

    // التحكم بالبحث عبر مفاتيح الأسهم والانتر
...
// تصدير جدول الأهداف إلى ملف Excel / CSV
function initCsvExport() {
    ...
    exportBtn.addEventListener('click', () => {
        ...
        monthsNames.forEach((month, idx) => {
            ...
            const mSales = firestoreRawData.sales.filter(s => parseMonthFromDate(s.date) === monthCode);
