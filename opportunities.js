...
window.addToActivityLog = addToActivityLog; // الإتاحة العامة للاستخدام المباشر في واجهة HTML
...
window.addProductRow = addProductRow; // إتاحتها كدالة عامة
...
window.calculateMainVisitValue = calculateMainVisitValue; // إتاحتها كدالة عامة
...
window.debouncedSaveSingleRow = debouncedSaveSingleRow; // إتاحتها كدالة عامة
...
window.handleBulkAction = handleBulkAction; // إتاحتها كدالة عامة
...
window.handleStatusChange = handleStatusChange; // إتاحتها كدالة عامة
...
/* ==========================================
   أكواد التقويم المخصص (نسخة الفرص البيعية المدمجة)
   ========================================== */
...
function changeMonth(step) {
    ...
    if (yearSelect) {
        let optionExists = Array.from(yearSelect.options).some(opt => parseInt(opt.value) === viewYear);
        if (!optionExists) {
            const opt = document.createElement('option');
            opt.value = viewYear;
            opt.textContent = viewYear;
            if (step < 0) {
...
window.openCustomDatePicker = function(event, el, rowId) {
    ...
    if (val && /^\d{4}-\d{2}-\d{2}$/.test(val)) {
        ...
        const yearSelect = document.getElementById('calYearSelect');
        if (yearSelect && !Array.from(yearSelect.options).some(opt => parseInt(opt.value) === viewYear)) {
            const opt = document.createElement('option');
            opt.value = viewYear;
            opt.textContent = viewYear;
            yearSelect.insertBefore(opt, yearSelect.firstChild);
...
// دالة مخصصة لربط التقويم بحفظ وتحديثات صفحة الفرص البيعية
...
// تهيئة تشغيل أحداث التقويم فور تحميل الملف
setupCalendarEvents();
loadLogsData();
