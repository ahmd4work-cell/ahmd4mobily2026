...
function updateStats() { 
    ...
    let total = rows.length; // الإجمالي الدائم لجميع الفرص كعدد
    let tDay = 0, tMonth = 0, valTotal = 0, valMonth = 0; 
    ...
    rows.forEach(row => { 
        ...
        // الجمع للقيم يحدث فقط للفرص المهتمة أو قيد التأهيل
        ...
        if (dateInput && dateInput.value) { 
            ...
            // حساب أعداد اليوم والشهر مخصص للحالات المهتمة فقط
            if (status === 'مهتم' || status === 'تأهيل لفرصة') {
                if (date === today) tDay++; 
