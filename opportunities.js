...
// عرض سجل النشاط العام بالاعتماد على الهيكلة الجديدة
function renderLogs(list) {
    ...
    list.slice(0, 100).forEach(log => {
        ...
        // معالجة رجعية للسجلات القديمة إن وجدت
...
// إضافة نشاط جديد متوافق مع Firebase ومع التصميم الجديد
async function addToActivityLog(fieldName, oldVal, newVal, companyName, ownerName) { 
    ...
    let actionText = '';
    if (fieldName === 'الحالة') {
        actionText = `تم تغير الحالة من ${escapeHTML(oldVal) || 'فارغ'} الى ${escapeHTML(newVal) || 'فارغ'} لـ ( ${escapeHTML(cleanCompany)} )`;
    } else if (fieldName === 'إجراء') {
        actionText = `${escapeHTML(oldVal)} لـ ( ${escapeHTML(cleanCompany)} )`;
    } else {
        actionText = `تعديل ${escapeHTML(fieldName)} من [${escapeHTML(oldVal) || 'فارغ'}] إلى [${escapeHTML(newVal) || 'فارغ'}] لـ ( ${escapeHTML(cleanCompany)} )`;
    }
    ...
    logsDataList = logsDataList.slice(0, 100); // الاحتفاظ بآخر 100 نشاط
    saveLogsLocalBackup();
    renderLogs(logsDataList);
...
// عرض الملاحظات داخل النافذة المنبثقة مع التوافق البصري 
function openNote(el) {
    ...
    if (historyLog) {
        historyLog.innerHTML = arr.map((msg, index) => {
            ...
            return `
            <div class="log-entry" style="padding-right:10px;">
                <span class="log-header-info">
                    <span>${escapeHTML(userName)}</span>
                    <span dir="ltr">${escapeHTML(dayStr)} ${escapeHTML(msg.date)}</span>
                    <span dir="ltr">${escapeHTML(msg.time)}</span>
                </span>
                <span class="log-sep">|</span>
                <span class="log-action" style="flex:1;">${escapeHTML(msg.text)}</span>
...
function reorderRows() { 
    ...
    // التعديل هنا: جعل الفرص بدون تاريخ ترتفع للأعلى
    rowsData.sort((a, b) => {
    ...
    // التعديل هنا: جعل مجموعة بدون تاريخ تظهر في أعلى القائمة
...
function applySelectedDate(dateStr) {
    if (activeInputTarget) {
        ...
        if (activeInputDisplayTarget) {
            activeInputDisplayTarget.value = formatDateToDisplay(dateStr);
        }
        if (activeRowTargetId) {
            updateEditDateField(document.getElementById(activeRowTargetId));
...
// ----------------------------------------------------
// التحديث الجديد: ربط جميع الدوال بكائن النافذة (window)
// لكي تكون متاحة للاستدعاء مباشرة من ملف HTML
// ----------------------------------------------------
window.initDatePicker = initDatePicker;
window.listenToOpportunities = listenToOpportunities;
