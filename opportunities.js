    ...
    onSnapshot(oppsRef, (snapshot) => {
        ...
        if (!snapshot.empty) {
            snapshot.forEach((docSnapshot) => {
                ...
                saveRowLocally(data.id); // تحديث التخزين المحلي بنسخة السحابة
            });
        } else {
            // استرجاع من الذاكرة المحلية كبديل آهل في حال فراغ السحابة أو انقطاع الاتصال
            try {
...
async function saveSingleRow(rowId) {
    saveRowLocally(rowId); // تأكيد الحفظ محلياً أولاً
    const row = document.getElementById(rowId);
    if (!row) return;
...
function debouncedSaveSingleRow(rowId) {
    saveRowLocally(rowId); // حفظ فوري في الذاكرة المحلية مع كل ضربة مفتاح
        ...
        if (result.isConfirmed) {
            for (let chk of selected) {
                ...
                if (row && row.id) {
                    ...
                    // إزالة الصف من التخزين المحلي أيضاً
                    try {
                        let localCache = JSON.parse(localStorage.getItem(OPP_LOCAL_KEY) || '{}');
