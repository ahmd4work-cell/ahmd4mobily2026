...
function initDatePicker() {
    ...
    dpMonthLabel.addEventListener("click", () => {
        ...
        dpMonthNames.forEach((name, index) => {
            const item = document.createElement("div"); item.className = "dp-dropdown-item"; item.textContent = name;
    ...
    dpYearLabel.addEventListener("click", () => {
        ...
        for (let y = 2000; y <= 2050; y++) {
            const item = document.createElement("div"); item.className = "dp-dropdown-item"; item.textContent = y;
    ...
    // إغلاق التقويم عند النقر في أي مكان فارغ بالصفحة
...
function updateDpTopInput() { if (dpSelectedDate) { dpTopInput.textContent = `${dpMonthNames[dpCurrentMonth]} ${dpSelectedDate}, ${dpCurrentYear}`; } else { dpTopInput.textContent = `${dpMonthNames[dpCurrentMonth]} ${dpCurrentYear}`; } }
...
function renderCustomCalendar() {
    ...
    for (let i = 0; i < firstDay; i++) {
        const emptyCell = document.createElement("div"); emptyCell.className = "dp-date dp-empty"; dpDatesGrid.appendChild(emptyCell);
    }
    for (let d = 1; d <= daysInMonth; d++) {
        const cell = document.createElement("div"); cell.className = "dp-date"; cell.textContent = d;
        const loopDateTime = new Date(dpCurrentYear, dpCurrentMonth, d).getTime();
        
        if (loopDateTime < currentTodayTime) cell.classList.add("dp-past");
        else if (loopDateTime === currentTodayTime) cell.classList.add("dp-today");
        if (d === dpSelectedDate) cell.classList.add("dp-selected");

        cell.addEventListener("click", (e) => { e.stopPropagation(); dpSelectedDate = d; renderCustomCalendar(); updateDpTopInput(); });
...
function openCustomDatePicker(e, inputEl, rowId) {
    if(e) e.stopPropagation(); dpActiveInput = inputEl; dpActiveRowId = rowId;
    if (inputEl.value) { const parts = inputEl.value.split('-'); if (parts.length === 3) { dpCurrentYear = parseInt(parts[0], 10); dpCurrentMonth = parseInt(parts[1], 10) - 1; dpSelectedDate = parseInt(parts[2], 10); } } 
    else { const d = new Date(); dpCurrentYear = d.getFullYear(); dpCurrentMonth = d.getMonth(); dpSelectedDate = null; }
    
    renderCustomCalendar();
    const popup = document.getElementById('datePickerPopup'); popup.style.display = 'block';
    const rect = inputEl.getBoundingClientRect(); let topPos = rect.bottom + window.scrollY + 4; let leftPos = rect.left + window.scrollX;
    
    setTimeout(() => {
        const popupRect = popup.getBoundingClientRect();
        if (leftPos + popupRect.width > window.innerWidth) leftPos = window.innerWidth - popupRect.width - 10;
        if (topPos + popupRect.height > window.innerHeight + window.scrollY) topPos = rect.top + window.scrollY - popupRect.height - 4; 
        popup.style.top = topPos + 'px'; popup.style.left = leftPos + 'px';
    }, 0);
}

function closeCustomDatePicker() {
    const popup = document.getElementById('datePickerPopup'); if(popup) popup.style.display = 'none';
    if(dpDropdown) dpDropdown.style.display = 'none'; dpActiveInput = null; dpActiveRowId = null;
}

function applySelectedDate(dateStr) {
    if (!dpActiveInput) return;
    const oldVal = dpActiveInput.dataset.old || ''; dpActiveInput.value = dateStr;
    ...
    if (mainRow) {
        ...
        dpActiveInput.dataset.old = dateStr; updateEditDateField(mainRow); saveSingleRow(dpActiveRowId); updateAllDateColors(); reorderRows(); 
    }
    closeCustomDatePicker();
}
