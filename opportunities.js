...
let saveTimeout;
let searchTimeout;
let initialScrollDone = false; // متغير للتحكم في التمرير التلقائي للشهر الحالي عند التحميل الأول
...
// دالة تحويل التاريخ للعرض بصيغة يوم-شهر-سنة
...
// حفظ النسخة الاحتياطية لسجل النشاط محلياً
...
// تحميل وترقيم سجل النشاط من التخزين المحلي والسحابة
...
// عرض سجل النشاط بشكل مطابق للعملاء
...
// إضافة حدث لسجل النشاط وحفظه سحابياً ومحلياً
...
// حفظ الصف محلياً بشكل فوري لمنع ضياع أي بيانات
...
function listenToOpportunities() {
    ...
    onSnapshot(oppsRef, (snapshot) => {
        ...
        // 1. حفظ حالة المربعات (تفاصيل المنتج) المفتوحة لكي لا تختفي
        let openSubTables = [];
        document.querySelectorAll('.sub-table-row').forEach(row => {
            if (row.style.display === 'table-row') {
                openSubTables.push(row.id);
            }
        });

        // 2. حفظ موقع مؤشر الكتابة بدقة لتجنب مقاطعة المستخدم
        let activeId = null;
        ...
        // 3. استعادة فتح مربعات التفاصيل التي كانت مفتوحة
        openSubTables.forEach(id => {
            ...
            if (sub) {
                ...
                arrows.forEach(arrow => arrow.className = 'fas fa-caret-down'); // قلب السهم للأسفل
            }
        });

        // 4. استعادة مؤشر الكتابة لنفس الحقل
...
// التعديل هنا: زيادة المهلة الزمنية لـ 5 ثواني
...
// التعديل الرئيسي لترتيب البيانات تنازلياً وتمرير الشاشة للشهر الحالي
function reorderRows() { 
    ...
    // الترتيب الأساسي بناءً على عمود التاريخ المتوقع (تنازلي: الأيام القادمة فوق والأيام القديمة تحت)
    rowsData.sort((a, b) => {
    ...
    // ترتيب الأشهر (تنازلي: الأشهر القادمة في الأعلى والأشهر الماضية في الأسفل)
    ...
    sortedMonths.forEach(month => { 
        ...
        // تمييز فاصل الشهر الحالي بـ ID محدد لعمل التمرير إليه لاحقاً
        if (isCurrentMonth) {
    ...
    // التمرير التلقائي للشهر الحالي عند التحميل لأول مرة فقط
    if (!initialScrollDone && rows.length > 0) {
        ...
        setTimeout(() => {
            ...
            if (currentMonthSep && tableWrapper) {
                ...
                // التمرير إلى فاصل الشهر الحالي مع خصم ارتفاع شريط العناوين (تقريباً 42px)
                ...
                // إذا لم يكن هناك بيانات تخص الشهر الحالي، نعتبر التمرير الأول منتهياً
                initialScrollDone = true;
            }
        }, 500); // إعطاء مهلة للتأكد من اكتمال رسم الـ DOM
    }
}
