...
function reorderRows() { 
    ...
    // الفرز يضع الفرص بدون تاريخ متوقع في أعلى الصفحة دائماً
...
function updateStats() { 
    ...
    let total = 0;    // الكل: عدد كل الفرص التي حالتها (مهتم)
    let tMonth = 0;   // هذا الشهر (مهتم): عدد الفرص حسب عمود تاريخ الفرصة
    let tDay = 0;     // اليوم (مهتم): عدد الفرص حسب عمود تاريخ الفرصة
    let valTotal = 0; // القيمة الإجمالية (مهتم): جميع قيم الفرص التي حالتها مهتم
    let valMonth = 0; // قيمة الشهر (مهتم): قيم الفرص التي حالتها مهتم حسب عمود التاريخ المتوقع
    ...
    rows.forEach(row => { 
        ...
        if (isInterested) {
            // 1. حساب إجمالي عدد الفرص التي حالتها مهتم
            total++;
            ...
            // 2. القيمة الإجمالية لجميع الفرص التي حالتها مهتم
            valTotal += visitVal;

            // 3. أعداد الفرص (هذا الشهر واليوم) بناءً على عمود (تاريخ الفرصة)
            ...
            // 4. قيمة الشهر بناءً على عمود (التاريخ المتوقع)
