...
function renderRow(v = {}, prepend = false) {
    ...
    mainRow.innerHTML = `
        ...
        <td>
            <input type="text" class="excel-input exp-date-input-display readonly-input" value="${formatDateToDisplay(expDate)}" readonly style="cursor:pointer;" onclick="openCustomDatePicker(event, this, '${rowId}')" placeholder="اختر التاريخ">
...
function reorderRows() { 
    ...
    const rowsData = rows.map(row => {
        ...
        return {
            ...
            date: isEmptyDate ? '9999-12-31' : dateVal,
    ...
    rowsData.forEach(item => { 
        const month = item.isEmptyDate ? 'بدون تاريخ متوقع' : item.date.substring(0, 7); 
