...
function openNote(el) {
    ...
    if (historyLog) {
        historyLog.innerHTML = arr.map((msg, index) => {
            ...
            // زر الحذف يظهر فقط إذا كان تاريخ الملاحظة يطابق تاريخ اليوم
            let deleteBtnHTML = '';
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            if (arr[index] && arr[index].date === getTodayFormatted()) {
                ...
                openNote(currentActivePreview); // إعادة تصيير النافذة للتحديث
            } else {
...
function initDatePicker() {
    ...
    monthSelect.addEventListener('change', (e) => { viewMonth = parseInt(e.target.value, 10); renderCustomCalendarDays(); });
    yearSelect.addEventListener('change', (e) => { viewYear = parseInt(e.target.value, 10); renderCustomCalendarDays(); });
    cancelBtn?.addEventListener('click', closeCustomDatePicker);
    nextBtn?.addEventListener('click', () => { if (activeInputTarget && tempSelectedDateStr) { applySelectedDate(tempSelectedDateStr); } else { closeCustomDatePicker(); } });
    overlay.addEventListener('click', (e) => { if (e.target === overlay) closeCustomDatePicker(); });
}
...
function renderCustomCalendarDays() {
    ...
    for (let i = 0; i < firstDayIndex; i++) { grid.appendChild(document.createElement('span')); }
    ...
    for (let day = 1; day <= totalDays; day++) {
        ...
        if (dayOfWeek === 5 || dayOfWeek === 6) daySpan.classList.add('weekend-number');
        if (dateStr === todayStr) daySpan.classList.add('today-day');
        if (tempSelectedDateStr === dateStr) daySpan.classList.add('selected-day');
...
Object.assign(window, {
    toggleSubTable, addProductRow, calculateMainVisitValue, openNote, closeNote, saveNote, handleStatusChange, deleteNote,
