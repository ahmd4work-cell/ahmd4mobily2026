// ==========================================
// opportunities.js - إدارة الفرص البيعية سحابياً (نسخة محسنة ومطورة)
// ==========================================
...
// دالة حماية النصوص لمنع تداخل علامات التنصيص والأكواد
...
function listenToOpportunities() {
    ...
    onSnapshot(oppsRef, (snapshot) => {
        ...
        // استخدام docChanges لتحديث الصفوف المتغيرة فقط وحماية مؤشر الكتابة (Focus)
...
function renderRow(v = {}, prepend = false) {
    ...
    if (document.getElementById(rowId)) return; // منع التكرار
    ...
    mainRow.innerHTML = `
        ...
        <td><input type="text" class="excel-input" value="${escapeHTML(v.comp)}" data-old="${escapeHTML(v.comp)}" onfocus="this.dataset.old=this.value" onkeyup="updateEditDateField(this.closest('tr')); debouncedSaveSingleRow('${rowId}');" onblur="addToActivityLog('الشركة', this.dataset.old, this.value, this.value); this.dataset.old=this.value;" onmouseenter="showStatusTooltip(this)" onmouseleave="hideStatusTooltip()"></td>
        <td><input type="text" class="excel-input" value="${escapeHTML(v.address)}" data-old="${escapeHTML(v.address)}" onfocus="this.dataset.old=this.value" onkeyup="updateEditDateField(this.closest('tr')); debouncedSaveSingleRow('${rowId}');" onblur="addToActivityLog('العنوان', this.dataset.old, this.value, this.closest('tr').cells[1].querySelector('input').value); this.dataset.old=this.value;"></td>
        <td><input type="text" class="excel-input" value="${escapeHTML(v.mgr)}" data-old="${escapeHTML(v.mgr)}" onfocus="this.dataset.old=this.value" onkeyup="updateEditDateField(this.closest('tr')); debouncedSaveSingleRow('${rowId}');" onblur="addToActivityLog('المسؤول', this.dataset.old, this.value, this.closest('tr').cells[1].querySelector('input').value); this.dataset.old=this.value;"></td>
        <td>
            <div class="phone-cell-container">
                ...
                <input type="text" class="excel-input" value="${escapeHTML(v.mob)}" data-old="${escapeHTML(v.mob)}" oninput="this.value = this.value.replace(/[^0-9]/g, ''); debouncedSaveSingleRow('${rowId}');" onfocus="this.dataset.old=this.value" onkeyup="updateEditDateField(this.closest('tr'));" onblur="addToActivityLog('رقم التواصل', this.dataset.old, this.value, this.closest('tr').cells[1].querySelector('input').value); this.dataset.old=this.value;">
            </div>
        </td>
        <td><input type="text" class="excel-input" value="${escapeHTML(v.email)}" data-old="${escapeHTML(v.email)}" onfocus="this.dataset.old=this.value" onkeyup="updateEditDateField(this.closest('tr')); debouncedSaveSingleRow('${rowId}');" onblur="addToActivityLog('الإيميل', this.dataset.old, this.value, this.closest('tr').cells[1].querySelector('input').value); this.dataset.old=this.value;"></td>
        <td><input type="text" class="excel-input" value="${escapeHTML(v.record)}" data-old="${escapeHTML(v.record)}" oninput="this.value = this.value.replace(/[^0-9]/g, ''); debouncedSaveSingleRow('${rowId}');" onfocus="this.dataset.old=this.value" onkeyup="updateEditDateField(this.closest('tr'));" onblur="addToActivityLog('السجل', this.dataset.old, this.value, this.closest('tr').cells[1].querySelector('input').value); this.dataset.old=this.value;"></td>
        <td><input type="text" class="excel-input readonly-input" value="${escapeHTML(oppDate)}" style="color:var(--text-muted); font-weight:700;" readonly><input type="hidden" class="opp-date-val" value="${escapeHTML(oppDate)}"></td>
        <td><input type="text" class="excel-input cur-serv-val" value="${escapeHTML(v.curServ)}" data-old="${escapeHTML(v.curServ)}" onfocus="this.dataset.old=this.value" onkeyup="updateEditDateField(this.closest('tr')); debouncedSaveSingleRow('${rowId}');" onblur="addToActivityLog('الخدمة', this.dataset.old, this.value, this.closest('tr').cells[1].querySelector('input').value); this.dataset.old=this.value;" onmouseenter="showStatusTooltip(this)" onmouseleave="hideStatusTooltip()"></td>
        <td><input type="number" class="excel-input opp-value-input readonly-input" value="${escapeHTML(v.oppValue)}" readonly style="color:var(--accent-blue); font-weight:800; cursor:not-allowed; background: transparent;"></td>
        <td><div class="notes-preview" onclick="openNote(this)" data-full-notes='${notesJson.replace(/'/g, "&apos;")}' id="preview-${Date.now()}">${escapeHTML(lastNoteText)}</div></td>
        <td>
        ...
        <td>
            <input type="text" class="excel-input exp-date-input readonly-input" value="${escapeHTML(v.expDate)}" data-old="${escapeHTML(v.expDate)}" readonly style="cursor:pointer;" onclick="openCustomDatePicker(event, this, '${rowId}')" placeholder="اختر التاريخ">
            <input type="hidden" class="edit-date-val" value="${escapeHTML(v.editDate)}">
        </td>
        <td><input type="text" class="excel-input" value="${escapeHTML(v.owner)}" data-old="${escapeHTML(v.owner)}" onfocus="this.dataset.old=this.value" onkeyup="updateEditDateField(this.closest('tr')); debouncedSaveSingleRow('${rowId}');" onblur="addToActivityLog('المالك', this.dataset.old, this.value, this.closest('tr').cells[1].querySelector('input').value); this.dataset.old=this.value;"></td>
    `;
...
// تحديث بيانات الصف القائم دون إعادة إنشاء الجدول بالكامل لتفادي فقدان المؤشر
function updateRowDataInDOM(v) {
    const mainRow = document.getElementById(v.id);
    if (!mainRow) return;

    // تحديث الحقول فقط إذا لم يكن المستخدم يكتب فيها حالياً
    const updateInputIfUnfocused = (inputEl, newVal) => {
        if (document.activeElement !== inputEl && inputEl) {
            inputEl.value = newVal || '';
            inputEl.dataset.old = newVal || '';
        }
    };

    updateInputIfUnfocused(mainRow.cells[1].querySelector('input'), v.comp);
    updateInputIfUnfocused(mainRow.cells[2].querySelector('input'), v.address);
    updateInputIfUnfocused(mainRow.cells[3].querySelector('input'), v.mgr);
    updateInputIfUnfocused(mainRow.cells[4].querySelector('input'), v.mob);
    updateInputIfUnfocused(mainRow.cells[5].querySelector('input'), v.email);
    updateInputIfUnfocused(mainRow.cells[6].querySelector('input'), v.record);
    updateInputIfUnfocused(mainRow.cells[8].querySelector('input'), v.curServ);
    updateInputIfUnfocused(mainRow.cells[13].querySelector('input'), v.owner);
    
    const statusSelect = mainRow.cells[11].querySelector('select');
...
function addProductRow(rowId, data = {}) {
    ...
    row.innerHTML = `
        ...
        <td><input type="number" class="prod-qty" min="0" value="${escapeHTML(data.qty)}" onkeyup="updateEditDateField(this.closest('.sub-table-row').previousElementSibling); calculateMainVisitValue('${rowId}')" oninput="calculateMainVisitValue('${rowId}')"></td>
        <td><input type="number" class="prod-sub" min="0" value="${escapeHTML(data.sub)}" onkeyup="updateEditDateField(this.closest('.sub-table-row').previousElementSibling); calculateMainVisitValue('${rowId}')" oninput="calculateMainVisitValue('${rowId}')"></td>
        <td><input type="number" class="prod-total readonly-input" value="${escapeHTML(data.total)}" readonly style="color:var(--text-muted); font-weight:700; cursor:not-allowed;"></td>
        ...
        historyLog.innerHTML = arr.map(msg => {
            ...
            <div class="log-entry" style="display: block; line-height: 1.6;">
                <div style="margin-bottom: 4px; display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                    ...
                    <span class="log-timestamp"><i class="fas fa-clock"></i> ${dayStr}${escapeHTML(msg.date)} ${escapeHTML(msg.time)}</span>
                </div>
                <div class="log-action" style="padding-right: 5px; color: #0f172a; font-size: 11px; font-weight: 700; white-space: pre-wrap;">${escapeHTML(msg.text)}</div>
            </div>
            `;
...
function addToActivityLog(fieldName, oldVal, newVal, companyName) { 
    ...
    const fullLogHTML = `
        <div class="log-entry">
            ...
            <span class="log-divider">|</span>
            <span class="log-action">${escapeHTML(actionText)}</span>
        </div>
    `; 
