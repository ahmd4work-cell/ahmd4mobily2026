...
// دالة جديدة لتحويل التاريخ إلى قيمة رقمية لعملية الفرز
...
function loadSalesFromStorage() {
    ...
    // ترتيب الصفوف حسب تاريخ الإنشاء (الأحدث في الأعلى، الأقدم في الأسفل)
    saved.sort((a, b) => {
        const dateA = parseDateToValue(a.date);
        const dateB = parseDateToValue(b.date);
        
        if (dateB !== dateA) {
            return dateB - dateA; // الأحدث أولاً
        }
        // في حال تساوي التاريخ، نرتب حسب المعرف
...
function renderTableRow(obj) {
    ...
    row.innerHTML = `
        ...
        <td><input type="text" class="excel-input readonly-input date-field" value="${formatDateDisplay(obj.date)}" readonly style="color:var(--text-muted); font-weight:700; text-align: center;" dir="ltr"></td>
        ...
        <td>
            <select class="excel-input status-select ${getStatusClass(obj.status)}" data-old="${obj.status || 'جديد'}" onchange="handleStatusChange(this, '${obj.id}', '${obj.comp}')">
        ...
        <td><input type="text" class="excel-input readonly-input last-mod-field date-field" value="${formatDateDisplay(obj.lastModifiedDate || '---')}" readonly style="color:var(--text-muted); font-weight:700; text-align: center;" dir="ltr"></td>
...
async function addOrderRow() {
    ...
    const newOrder = {
        ...
        comp: comp,
        cr: cr,
        status: 'جديد', // تعديل الحالة الافتراضية
