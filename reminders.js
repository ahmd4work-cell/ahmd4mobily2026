...
function renderGrid() {
    ...
    // إنشاء المربع رقم 12 (فارغ ويحتوي على نص للإضافة)
    const addNewCard = document.createElement('div');
    addNewCard.className = 'reminder-card add-new-card';
    addNewCard.innerHTML = `
        <div class="add-new-content">
            <i class="fas fa-plus-circle"></i> اضافة مذكرة جديد
        </div>
    `;
...
function setupCalendarEvents() {
    ...
    const clearBtn = document.getElementById('calClearBtn'); // الزر الجديد
...
function renderCalendarDays() {
    ...
    // الحصول على تاريخ اليوم لتمييزه
    const today = new Date();
...
window.saveData = function() {
    // نجمع بيانات الـ 11 بطاقة الفعلية فقط
    const cards = document.querySelectorAll('.reminder-card:not(.add-new-card)');
    const data = [];
...
function sortCardsByDate() {
    ...
    // جلب الـ 11 بطاقة العادية فقط
    const cardsArray = Array.from(grid.querySelectorAll('.reminder-card:not(.add-new-card)'));
    // جلب البطاقة رقم 12 (البطاقة الفارغة للإضافة)
    ...
    // إعادة إرفاق البطاقات المرتبة
    cardsArray.forEach(card => grid.appendChild(card));
    // إرجاع بطاقة (إضافة مذكرة جديد) لتكون دائماً في آخر الشبكة
    if (addNewCard) grid.appendChild(addNewCard);
}
