...
function setupCalendarEvents() {
    ...
    // عند تغيير الشهر من القائمة
    monthSelect.addEventListener('change', (e) => {
        viewMonth = parseInt(e.target.value, 10);
        renderCalendarDays();
    });

    // عند تغيير السنة من القائمة
...
function openCustomCalendar(inputEl) {
    ...
    // تحديث قيم القوائم المنسدلة العلوية
    const monthSelect = document.getElementById('calMonthSelect');
    const yearSelect = document.getElementById('calYearSelect');
...
function renderCalendarDays() {
    ...
    for (let day = 1; day <= totalDays; day++) {
        ...
        // تمييز يومي الجمعة والسبت باللون الأحمر وبخط غير عريض
