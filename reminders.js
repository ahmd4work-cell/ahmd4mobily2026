...
/* ==========================================
   وظائف مسح المذكرات (محدثة مع التأكيد)
   ========================================== */

window.clearNote = function(btnElement) {
    // تحديد المذكرة المراد حذفها وعرض النافذة المنبثقة
...
function setupDeleteConfirmation() {
    ...
    if (confirmBtn) {
        confirmBtn.addEventListener('click', () => {
            if (noteToDelete) {
                noteToDelete.querySelector('.card-title-input').value = '';
                noteToDelete.querySelector('.card-date-input').value = '';
                noteToDelete.querySelector('.card-textarea').value = '';
...
function setupCalendarEvents() {
    ...
    // التنقل بالسهمين (-1 للسابق، +1 للتالي)
...
function changeMonth(step) {
    ...
    // التأكد من أن السنة موجودة في قائمة الخيارات، وإن لم تكن تضاف برمجياً
    if (yearSelect) {
...
window.openCustomCalendar = function(inputEl) {
    ...
    if (val && /^\d{4}-\d{2}-\d{2}$/.test(val)) {
        ...
        // التحقق من إضافة السنة إذا كانت أقدم من الحالية
        const yearSelect = document.getElementById('calYearSelect');
