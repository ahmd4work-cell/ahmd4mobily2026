...
// دالة جلب آخر ملاحظة لعرضها كنص مختصر داخل الجدول
function getLastNoteOnlyFromJSON(jsonStr) { 
    try { 
        const arr = JSON.parse(jsonStr); 
        return (arr && arr.length > 0) ? arr[arr.length - 1].text : "أضف ملاحظة..."; 
    } catch(e) { 
        return "أضف ملاحظة..."; 
    } 
}
...
function renderProducts(filtered = null) {
    ...
    items.forEach((p) => {
        ...
        const rawNote = p.rowNote || '';
        let notesJsonStr = "[]";
        if (rawNote) {
            if (typeof rawNote === 'string' && rawNote.trim().startsWith('[')) {
                notesJsonStr = rawNote;
            } else {
                notesJsonStr = JSON.stringify([{ user: "المستخدم", date: p.date || getTodayFormatted(), time: "00:00", text: rawNote }]);
            }
        }
        const shortNoteText = getLastNoteOnlyFromJSON(notesJsonStr);
        ...
        tbody.innerHTML += `<tr class="${isLocked ? 'row-locked' : ''}">
            ...
            <td class="not-locked">
                <div class="notes-preview" onclick="openNote(this)" data-index="${p.originalIndex}" data-full-notes="${escapeHTML(notesJsonStr)}">${escapeHTML(shortNoteText)}</div>
            </td>
            </tr>`;
    });
...
async function updateField(idx, f, v) {
    ...
    if(!item) return;
    item[f] = typeof v === 'string' ? v.trim() : v; 
...
// ==========================================
// إدارة الملاحظات (الملاحظات العامة وملاحظات الصفوف)
// ==========================================
        ...
        historyLog.innerHTML = arr.map((msg, index) => {
            ...
            if (msg.date && msg.time) {
                ...
                if (!isNaN(noteDateTime.getTime())) {
                    let diffInHours = (new Date().getTime() - noteDateTime.getTime()) / (1000 * 60 * 60);
                    if (diffInHours > 24) {
                        showDelete = false; 
                    }
                }
...
async function saveNote() {
    ...
    if (txt && currentActivePreview) {
        ...
        const oldLastNote = getLastNoteOnlyFromJSON(JSON.stringify(arr));
        let username = "أحمد"; 
        ...
        const idx = parseInt(currentActivePreview.getAttribute('data-index'));
        if (!isNaN(idx)) {
            await updateField(idx, 'rowNote', jsonStr);
            let dbData = JSON.parse(localStorage.getItem('asgate_products_db') || '{}')[currentOrderId] || [];
            let item = dbData[idx];
            let pIden = item ? (item.mobile || item.serial || item.name) : '';
            addToActivityLog('سجل المتابعة', oldLastNote, txt, pIden);
        }
        closeNote();
    } else if (!currentActivePreview) {
        saveGlobalNote();
    } else {
        closeNote();
    }
...
async function deleteNote(index) {
    ...
    if (window.Swal) {
        ...
        isConfirmed = confirm("هل أنت متأكد من حذف هذه الملاحظة؟");
    }
    ...
    if (isConfirmed) {
        ...
        const jsonStr = JSON.stringify(arr);
        currentActivePreview.setAttribute('data-full-notes', jsonStr);
        const newLastNote = getLastNoteOnlyFromJSON(jsonStr);
        currentActivePreview.innerText = newLastNote;
        ...
        if (!isNaN(idx)) {
            ...
            let item = dbData[idx];
            let pIden = item ? (item.mobile || item.serial || item.name) : '';
            addToActivityLog('سجل المتابعة', oldLastNote, newLastNote, pIden);
        }
...
function saveGlobalNote() {
    ...
    if (newText) {
        ...
        let newEntry = `<div style="width: 100%; display: flex; align-items: center; gap: 8px; margin-bottom: 6px;">
                        ...
                        <div class="log-action" style="width: 100%; display: block; white-space: pre-wrap; font-size: 13px; color: var(--text-dark); padding-right: 5px;">${escapeHTML(newText)}</div>`;
...
function handleFileUpload(event) {
    ...
    let newEntry = `<div style="width: 100%; display: flex; align-items: center; gap: 8px; margin-bottom: 6px;">
                    ...
                    <div class="log-action" style="width: 100%; display: block; color: var(--accent-blue); padding-right: 5px;">
                        <i class="fas fa-file-alt"></i> تم إرفاق ملف: ${escapeHTML(fileName)}
                    </div>`;
