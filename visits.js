    ...
    subRow.innerHTML = `
        <td colspan="14" style="padding:15px 10px; background:#f8fafc; box-shadow: inset 0 2px 4px rgba(0,0,0,.02);">
            <div style="display: flex; gap: 15px; align-items: stretch;">
                ...
                <div class="sub-table-date-card" style="width: 250px; background: white; border: 1px solid var(--border-soft); border-radius: 8px; padding: 10px; display: flex; flex-direction: column; justify-content: center; align-items: center; box-shadow: 0 4px 6px rgba(0,0,0,.05);">
                    <div class="date-card-title" style="font-weight:bold; color:#2e1065; margin-bottom:10px; font-size:12px;">تاريخ التعديل:</div>
...
function renderCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        const currentDayOfWeek = new Date(y, m, day).getDay();
        ...
        dayDiv.onclick = () => {
            if (!currentPickerRowId) return;
            const formattedDate = `${String(day).padStart(2, '0')}-${String(m + 1).padStart(2, '0')}-${y}`;
            applyDateToRow(currentPickerRowId, formattedDate);
            closeDatePicker();
        };
