...
function listenToVisits() {
    ...
    onSnapshot(visitsRef, (snapshot) => {
        ...
        // تحديث التخزين المحلي لدعم البحث الشامل عبر النظام بما فيها السجل الرئيسي
...
// دالة الترحيل لجدول الفرص البيعية
async function migrateToOpportunities(rowId) {
    const row = document.getElementById(rowId);
    if (!row) return;

    // استخراج بيانات الزيارة الأساسية
    const compVal = row.cells[1].querySelector('input').value;
    ...
    // استخراج المنتجات الفرعية إن وجدت
    const subRow = document.getElementById('sub-' + rowId);
    const products = [];
    if (subRow) {
        subRow.querySelectorAll('.product-body tr').forEach(pRow => {
    ...
    try {
        // حفظ السجل في مجموعة Opportunities في Firestore
        await setDoc(doc(db, "opportunities", newOppId), oppData);

        // حذف الزيارة من مجموعة visits بعد نجاح حفظها في الفرص البيعية
        await deleteDoc(doc(db, "visits", rowId));

        // تحديث التخزين المحلي للفرص والزيارات
...
function initGlobalSearch() {
    ...
    input.addEventListener('input', function() {
        ...
        // جلب البيانات من LocalStorage لجميع الصفحات المتعلقة بالبحث الشامل
        const sources = [
        ...
        sources.forEach(source => {
            ...
            data.forEach(item => {
                // تجميع كل الحقول المطلوبة للبحث (متضمنة السجل والكود والرقم)
                const searchableText = [
                    item.company || item.comp || '', 
                    item.responsible || item.mgr || '', 
                    item.phone || item.mob || '', 
                    item.email || '', 
                    item.record || '', 
...
function renderSearchResults(results, dropdown) {
    dropdown.innerHTML = '';
    if (results.length === 0) {
        dropdown.innerHTML = '<div class="search-no-results">لا توجد نتائج مطابقة لبحثك</div>';
    } else {
        // نعرض أول 15 نتيجة فقط كحد أقصى للحفاظ على الأداء
        results.slice(0, 15).forEach(res => {
            const el = document.createElement('a');
            el.href = `${res.source.link}#${res.id}`;
            el.className = 'search-result-item';
            
            const companyName = res.company || res.comp || 'بدون اسم';
            const responsible = res.responsible || res.mgr || '';
            const phone = res.phone || res.mob || '';
            const record = res.record || '';
            const code = res.code || res.customerCode || res.orderNo || res.orderNumber || '';
            
            el.innerHTML = `
                <div class="result-header">
                    <span class="result-title">${escapeHTML(companyName)}</span>
                    <span class="result-badge ${res.source.badge}">${res.source.name}</span>
                </div>
                <div class="result-details">
                    ${responsible ? `<span><i class="fas fa-user"></i> ${escapeHTML(responsible)}</span>` : ''}
                    ${phone ? `<span><i class="fas fa-phone"></i> ${escapeHTML(phone)}</span>` : ''}
                    ${record ? `<span><i class="fas fa-file-alt"></i> ${escapeHTML(record)}</span>` : ''}
                    ${code ? `<span><i class="fas fa-hashtag"></i> ${escapeHTML(code)}</span>` : ''}
                </div>
            `;
