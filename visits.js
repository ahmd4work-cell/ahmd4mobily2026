    ...
    onSnapshot(visitsRef, (snapshot) => {
        ...
        if (document.activeElement && (document.activeElement.tagName === 'INPUT' || document.activeElement.tagName === 'SELECT')) {
            ...
            if (subTableRow) {
                ...
                if (productRow && productTd) {
                    activeElementData = {
                        type: 'sub',
                        subId: subTableRow.id, 
                        rowIndex: productRow.rowIndex, 
                        cellIndex: productTd.cellIndex,
        ...
        if (activeElementData) {
            if (activeElementData.type === 'main') {
                ...
                if (activeRow && activeRow.cells[activeElementData.cellIndex]) {
                    ...
                    if (inputToFocus) {
                        inputToFocus.focus();
                        try { inputToFocus.setSelectionRange(activeElementData.selectionStart, activeElementData.selectionStart); } catch(e){}
                    }
                }
...
function renderRow(v = {}) {
    ...
    mainRow.innerHTML = `
        ...
        <td><input type="text" class="excel-input" value="${v.comp || ''}" data-old="${v.comp || ''}" onfocus="this.dataset.old=this.value" onkeyup="updateEditDateField(this.closest('tr'));" onchange="saveSingleRow('${rowId}');" onblur="addToActivityLog('الشركة', this.dataset.old, this.value, this.value); this.dataset.old=this.value;"></td>
        ...
        <td><input type="text" class="excel-input readonly-input" value="${visitDate}" style="color:var(--text-muted); font-weight:700;" readonly><input type="hidden" class="visit-date-val opp-date-val" value="${visitDate}"></td>
        <td><input type="text" class="excel-input cur-serv-val" value="${v.curServ || ''}" data-old="${v.curServ || ''}" onfocus="this.dataset.old=this.value" onkeyup="updateEditDateField(this.closest('tr'));" onchange="saveSingleRow('${rowId}');" onblur="addToActivityLog('الخدمة', this.dataset.old, this.value, this.closest('tr').cells[1].querySelector('input').value); this.dataset.old=this.value;"></td>
...
function applyStatusColor(selectEl) { 
    ...
    } else if (val === 'غير مهتم' || val === 'فقدان') { 
...
function updateEditDateField(row) { if (!row) return; const dateFormatted = getTodayFormatted(); const time24 = getTimeFormatted(); const hiddenInput = row.querySelector('.edit-date-val'); if (hiddenInput) hiddenInput.value = `${dateFormatted} ${time24}`; const editMains = row.querySelector('.edit-date-container-main'); if(editMains) editMains.innerHTML = `<span class="edit-date-d">${dateFormatted}</span><span class="edit-date-t">${time24}</span>`; const subRow = document.getElementById('sub-' + row.id); if (subRow) { const subContainer = subRow.querySelector('.edit-date-container-sub'); if (subContainer) subContainer.innerHTML = `<span class="edit-date-d">${dateFormatted}</span><span class="edit-date-t">${time24}</span>`; } }
    ...
    if (action === 'حذف') {
        ...
        if (result.isConfirmed) {
            for (let chk of selected) {
                ...
                if (row && row.id) {
                    await deleteDoc(doc(db, "visits", row.id));
                }
            }
...
function addToActivityLog(fieldName, oldVal, newVal, companyName) { 
    ...
    const fullLogHTML = `<div class="log-entry"><span class="log-badge-user"><i class="fas fa-user-circle"></i> المستخدم</span><span class="log-divider">|</span><span class="log-timestamp"><i class="fas fa-clock"></i> ${dayName} ${yyyy}-${mm}-${dd} ${timeStr}</span><span class="log-divider">|</span><span class="log-action">${actionText}</span></div>`; 
...
Object.assign(window, {
    ...
    toggleSubTable,
    addProductRow,
    calculateMainVisitValue,
    saveSingleRow,
    debouncedSaveSingleRow,
    updateEditDateField,
    addToActivityLog,
    openNote,
    closeNote,
