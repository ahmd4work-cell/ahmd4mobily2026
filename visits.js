...
function renderRow(v = {}) {
    ...
    mainRow.innerHTML = `
        ...
        <td>
            <select class="excel-input status-select" data-old="${escapeHTML(v.status || '')}" onchange="applyStatusColor(this); updateEditDateField(this.closest('tr')); debouncedSaveSingleRow('${rowId}'); addToActivityLog('الحالة', this.dataset.old, this.value, this.value); this.dataset.old=this.value;">
                ...
                <option value="مؤجلة" ${v.status === 'مؤجلة' ? 'selected' : ''}>مؤجلة</option>
...
function applyStatusColor(selectEl) {
    ...
    if (val === 'تمت الزيارة') selectEl.classList.add('status-green');
    else if (val === 'مجدولة') selectEl.classList.add('status-yellow-fff');
    else if (val === 'مؤجلة') selectEl.classList.add('status-gray-a5');
    else if (val === 'ملغاة') selectEl.classList.add('status-red-c00');
}
