...
function getLastNoteOnlyFromJSON(jsonString) {
    try {
        const arr = JSON.parse(jsonString || "[]");
        if (arr.length > 0) return arr[arr.length - 1].text;
    } catch(e) {}
    return "لا توجد ملاحظات";
}

// --- دوال Firebase وإدارة البيانات ---
...
function renderRow(v = {}) {
    ...
    mainRow.innerHTML = `
        ...
        <td><div class="notes-preview" onclick="openNote(this)" data-full-notes='${escapeHTML(notesJson)}' id="preview-${rowId}">${escapeHTML(lastNoteText)}</div></td>
        <td>
        ...
        historyLog.innerHTML = arr.map((msg, index) => { 
            ...
            if (msg.date && msg.time) {
                try {
                    ...
                    const diffInHours = (new Date() - noteDateTime) / (1000 * 60 * 60);
                    if (diffInHours > 24) {
                        showDelete = false; 
                    }
                } catch(e) {}
            ...
            let deleteBtnHtml = (showDelete && rowId) ? `<i class="fas fa-trash-alt delete-note-btn" onclick="deleteSpecificNote('${rowId}', ${index})" title="حذف الملاحظة"></i>` : '';
...
async function handleBulkAction(action) {
    ...
    if (action === 'حذف') {
        ...
        Swal.fire({
            icon: 'info', 
            title: 'قيد التطوير',
            text: `ميزة (${action}) سيتم تفعيلها قريباً!`, 
...
// --- دالة التحديث الإحصائي (تمت معالجة القطع وإكمالها بشكل صحيح) ---
...
function updateStats() { 
    ...
    if (document.getElementById('stat-value-total')) document.getElementById('stat-value-total').innerText = valTotal.toLocaleString();
    if (document.getElementById('stat-value-month')) document.getElementById('stat-value-month').innerText = valMonth.toLocaleString();
}

// --- دوال إضافية (تم بناء وبرمجة النواقص) ---
...
function addToActivityLog(field, oldVal, newVal, context) {
    ...
    if (logs.length > 30) logs.pop(); // الحفاظ على آخر 30 سجل منعاً للثقل
...
function renderActivityLog() {
    ...
    list.innerHTML = logs.map(log => `
        <div class="log-entry">
            <span class="log-header-info">${escapeHTML(log.date)} ${escapeHTML(log.time)}</span>
            <span class="log-sep">|</span>
            <span class="log-action">تعديل [ ${escapeHTML(log.field)} ] لعميل (${escapeHTML(log.context)}) من "${escapeHTML(log.oldVal)}" إلى "${escapeHTML(log.newVal)}"</span>
        </div>
    `).join('');
}

function openWhatsAppChat(btn) {
    const row = btn.closest('tr');
    let phone = row.cells[4].querySelector('input').value;
    if (!phone) return;
    
    phone = phone.replace(/\D/g, '');
    
    // إضافة رمز الاتصال السعودي بشكل ذكي وافتراضي إذا لم يتم إرفاقه
    if (phone.startsWith('0')) {
        phone = '966' + phone.substring(1); 
    } else if (!phone.startsWith('966') && phone.length <= 10) {
        phone = '966' + phone;
    }
...
// --- نظام التقويم (الـ DatePicker المفقود تم تصميمه بالكامل) ---

function openDatePicker(rowId) {
    activeDateRowId = rowId;
...
function closeDatePicker() {
    ...
    activeDateRowId = null;
}

function setTodayDate() {
    if (activeDateRowId) {
        const row = document.getElementById(activeDateRowId);
        if (row) {
            const dateFormatted = getTodayFormatted();
            const hiddenInput = row.querySelector('.visit-date-val');
            if (hiddenInput) hiddenInput.value = dateFormatted;
            
            const readonlyInput = row.querySelector('.visit-date-val').previousElementSibling;
            if (readonlyInput) readonlyInput.value = dateFormatted;
            
            saveSingleRow(activeDateRowId);
            updateEditDateField(row);
        }
    }
    closeDatePicker();
...
function initDatePicker() {
    ...
    const mSelect = document.getElementById('dpMonth');
    const ySelect = document.getElementById('dpYear');
    ...
    let yearsHtml = '';
    for(let y = curYear - 5; y <= curYear + 5; y++) yearsHtml += `<option value="${y}">${y}</option>`;
    ySelect.innerHTML = yearsHtml;
    
    mSelect.value = new Date().getMonth();
    ySelect.value = curYear;
    
    mSelect.onchange = renderCalendarDays;
    ySelect.onchange = renderCalendarDays;
...
function renderCalendarDays() {
    ...
    if (!mSelect || !ySelect || !daysGrid) return;
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        const dayClass = isToday ? 'day-number today-day' : 'day-number';
        html += `<div class="${dayClass}" onclick="selectDateFromPicker(${d}, ${month}, ${year})">${d}</div>`;
    }
    
    daysGrid.innerHTML = html;
}

// دالة نداء من التقويم لنقل التاريخ للخلايا
window.selectDateFromPicker = function(d, m, y) {
    if (activeDateRowId) {
        ...
        if (row) {
            ...
            const readonlyInput = row.querySelector('.visit-date-val').previousElementSibling;
            if (readonlyInput) readonlyInput.value = dateStr;
...
// ==========================================
// تصدير الدوال لكائن Window ليتمكن الـ HTML من قراءتها
// نظراً لاستخدام type="module" يجب ربط الدوال بالنطاق العام
// ==========================================
...
// التهيئة عند التشغيل (Initialization)
document.addEventListener('DOMContentLoaded', () => {
    listenToVisits();
    renderActivityLog();
    
    // إخفاء القوائم المنسدلة عند النقر بالخارج
