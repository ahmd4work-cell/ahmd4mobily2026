...
function renderRow(v = {}) {
    ...
    // دمج الملاحظات مع التخزين المحلي كصمام أمان
    let notesJson = v.notes || "[]";
    if (v.id) {
        try {
            const localNotes = localStorage.getItem('visit_notes_local_' + v.id);
...
function openNote(el) {
    ...
    // فحص التخزين المحلي لاسترجاع أحدث ملاحظة احتياطية إن وجدت
...
function saveNote() { 
    ...
    if (txt && currentActivePreview) { 
        ...
        // الحفظ المحلي الفوري في المتصفح (LocalStorage) لضمان عدم ضياع النص أبداً
        if (mainRow && mainRow.id) {
            try {
                localStorage.setItem('visit_notes_local_' + mainRow.id, jsonNotes);
            } catch(e) {
                console.error("خطأ في تخزين الملاحظة محلياً:", e);
            }
        }
...
function addToActivityLog(fieldName, oldVal, newVal, companyName) { 
    ...
    let actionText = fieldName === 'إجراء' ? `${oldVal} لزيارة شركة ( ${escapeHTML(cleanCompany)} )` : `تعديل ${fieldName} من [${escapeHTML(oldVal) || 'فارغ'}] إلى [${escapeHTML(newVal) || 'فارغ'}] لزيارة العميل ( ${escapeHTML(cleanCompany)} )`; 
