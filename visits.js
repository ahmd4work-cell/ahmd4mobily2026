                    ...
                    <table class="inner-table" style="width: 100%;">
                        <thead>
                            <tr>
                                ...
                                <th style="width:75px"><button class="header-plus-btn" type="button" onclick="addProductRow('${rowId}')" title="إضافة منتج"><i class="fas fa-plus"></i></button></th>
                            </tr>
                        </thead>
...
function addProductRow(rowId, data = {}) {
    ...
    row.innerHTML = `
        ...
        <td><div style="display:flex; justify-content:center;"><button class="sub-action-btn" type="button" title="حذف" onclick="if(this.closest('tbody').rows.length > 1) { const main = this.closest('.sub-table-row').previousElementSibling; updateEditDateField(main); this.closest('tr').remove(); calculateMainVisitValue('${rowId}', true); }"><i class="fas fa-trash-alt" style="font-size:10px;"></i></button></div></td>
    `;
}
...
async function migrateToOpportunities(rowId) {
    const row = document.getElementById(rowId);
    if (!row) return;

    const compVal = row.cells[1].querySelector('input').value;
    const address = row.cells[2].querySelector('input').value;
    const mgr = row.cells[3].querySelector('input').value;
    const mob = row.cells[4].querySelector('input').value;
    const email = row.cells[5].querySelector('input').value;
    const record = row.cells[6].querySelector('input').value;
    const visitDate = row.querySelector('.visit-date-val').value;
    const curServ = row.cells[8].querySelector('input').value;
    const oppValue = row.cells[9].querySelector('input').value;
    const notes = row.cells[10].querySelector('.notes-preview').getAttribute('data-full-notes') || '[]';
    ...
    const oppData = {
        ...
        products: products,
        status: 'فرصة جديدة',
        migrationDate: `${getTodayFormatted()} ${getTimeFormatted()}`
    };

    try {
        await setDoc(doc(db, "opportunities", newOppId), oppData);
        await deleteDoc(doc(db, "visits", rowId));

        const localOpps = JSON.parse(localStorage.getItem('crm_opportunities') || '[]');
        localOpps.push({ 
            id: newOppId, 
            company: compVal, 
            responsible: mgr, 
        ...
        localStorage.setItem('crm_opportunities', JSON.stringify(localOpps));

        const localVisits = JSON.parse(localStorage.getItem('crm_visits') || '[]');
        localStorage.setItem('crm_visits', JSON.stringify(localVisits.filter(v => v.id !== rowId)));

        Swal.fire('تم بنجاح', 'تم نقل الزيارة إلى الفرص البيعية بنجاح', 'success');
        addToActivityLog('نقل', 'زيارة', 'فرصة بيعية', compVal);

    } catch (error) {
        console.error("خطأ في النقل للفرص:", error);
        Swal.fire('خطأ', 'حدث مشكلة أثناء محاولة النقل للفرص البيعية', 'error');
    }
}
...
async function handleStatusChange(selectEl, rowId) {
    ...
    if (newVal === 'تأهيل لفرصة' && oldVal !== 'تأهيل لفرصة') {
        Swal.fire({
            title: 'نقل البيانات؟',
            text: 'هل تريد نقل بيانات هذه الزيارة إلى صفحة الفرص البيعية وحذفها من هنا؟',
            icon: 'question',
...
function exportSelectedToExcel(selectedIds) {
    const headers = [
        'الشركة', 'العنوان', 'المسؤول', 'رقم التواصل', 'البريد الإلكتروني', 
        'السجل الرئيسي', 'تاريخ الزيارة', 'الخدمة', 'القيمة', 'الملاحظات', 
        'الحالة', 'آخر تعديل', 'المالك'
    ];
    ...
    selectedIds.forEach(id => {
        ...
        const email = row.cells[5].querySelector('input')?.value || '';
        const record = row.cells[6].querySelector('input')?.value || '';
        const visitDate = row.querySelector('.visit-date-val')?.value || row.cells[7].querySelector('input.excel-input')?.value || '';
        const curServ = row.cells[8].querySelector('input')?.value || '';
        const oppValue = row.cells[9].querySelector('input')?.value || '';
        const notesJson = row.cells[10].querySelector('.notes-preview')?.getAttribute('data-full-notes') || '[]';
        const lastNote = getLastNoteOnlyFromJSON(notesJson);
        const status = row.cells[11].querySelector('select')?.value || '';
        const editDate = row.querySelector('.edit-date-val')?.value || '';
        const owner = row.cells[13].querySelector('input')?.value || '';

        rowsData.push([
            comp, address, mgr, mob, email, record, visitDate, curServ, oppValue, lastNote, status, editDate, owner
        ]);
    });
    ...
    link.href = url;
    link.setAttribute('download', `زيارات_محددة_${getTodayFormatted()}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);

    Swal.fire('تمالتصدير', 'تم تصدير البيانات بنجاح إلى ملف إكسيل', 'success');
}

function printSelectedRows(selectedIds) {
    const allMainRows = document.querySelectorAll('#tableBody tr.main-row');
    
    allMainRows.forEach(row => {
        if (!selectedIds.includes(row.id)) {
            row.classList.add('print-hide');
            const subRow = document.getElementById('sub-' + row.id);
            if (subRow) subRow.classList.add('print-hide');
        }
    });
    ...
    allMainRows.forEach(row => {
        ...
        const subRow = document.getElementById('sub-' + row.id);
        if (subRow) subRow.classList.remove('print-hide');
    });
}
...
window.handleBulkAction = function(action) {
    ...
    // إخفاء القائمة المنبثقة فور الاختيار
    ...
    if (action === 'حذف') {
        ...
        exportSelectedToExcel(selected);
    } else if (action === 'طباعة') {
        printSelectedRows(selected);
    }
};
...
// ==========================================
// نظام البحث الشامل (يجلب بيانات من كل الصفحات)
// ==========================================
function initGlobalSearch() {
    ...
    input.addEventListener('input', function() {
        ...
        const sources = [
            { key: 'crm_visits', name: 'زيارة', badge: 'badge-visits', link: 'visits.html' },
            { key: 'crm_opportunities', name: 'فرصة بيعية', badge: 'badge-opportunities', link: 'opportunities.html' },
            { key: 'crm_customers', name: 'عميل', badge: 'badge-customers', link: 'customers.html' },
            { key: 'crm_sales', name: 'مبيعات', badge: 'badge-sales', link: 'sales.html' }
        ];
        ...
        sources.forEach(src => {
            try {
                ...
                items.forEach(item => {
                    ...
                    const record = (item.record || '').toLowerCase();

                    if (comp.includes(query) || resp.includes(query) || phone.includes(query) || email.includes(query) || record.includes(query)) {
                        results.push({
                            title: item.company || 'بدون عنوان',
            ...
            dropdown.innerHTML = results.slice(0, 10).map(r => `
                <a href="${r.link}" class="search-result-item">
                    <div class="result-header">
                        ...
                        <span class="result-badge ${r.badge}">${r.sourceName}</span>
                    </div>
                    <div class="result-details">
                        <span><i class="fas fa-user"></i> ${escapeHTML(r.resp)}</span>
...
// تهيئة التهيئة عند تحميل الصفحة
