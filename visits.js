// ==========================================
// visits.js - إدارة الزيارات سحابياً ومحلياً (مع ربط الدوال بالنطاق العام window)
// ==========================================
...
function applyStatusColor(selectEl) { 
    ...
    if (mainRow) mainRow.classList.remove('row-shrink', 'row-uninterested', 'row-lost'); 
    ...
    if (val === 'تأهيل لفرصة') {
        ...
        if (mainRow) mainRow.classList.add('row-shrink', 'row-uninterested'); 
    } else if (val === 'فقدان') { 
        selectEl.classList.add('status-red-c00'); 
        if (mainRow) mainRow.classList.add('row-shrink', 'row-lost'); 
    } 
}
        ...
        historyLog.innerHTML = arr.map((msg, index) => { 
            ...
            if (msg.date && msg.time) {
                try {
                    ...
                    let noteDateTime;
                    if (parts[0].length === 4) {
                        noteDateTime = new Date(parts[0], parts[1] - 1, parts[2], timeParts[0], timeParts[1]);
                    } else {
                        noteDateTime = new Date(parts[2], parts[1] - 1, parts[0], timeParts[0], timeParts[1]);
                    }
    ...
    if (action === 'حذف') {
        ...
        const { value: newOwner } = await Swal.fire({
            ...
            inputValidator: (value) => {
                if (!value) return 'يرجى إدخال اسم المالك!';
            }
        });
        ...
        if (newOwner) {
            try {
                ...
                selected.forEach(chk => {
                    const row = chk.closest('tr');
                    if (row && row.id) {
                        const rowRef = doc(db, "visits", row.id);
                        batch.update(rowRef, { owner: newOwner, editDate: `${getTodayFormatted()} ${getTimeFormatted()}` });
                    }
                });
                await batch.commit();
                Swal.fire({icon: 'success', title: 'تم التحديث بنجاح', showConfirmButton: false, timer: 1500});
            } catch (e) {
                console.error("خطأ أثناء تغيير المالك الجماعي:", e);
                Swal.fire('خطأ', 'حدث خطأ أثناء تحديث المالك.', 'error');
            }
        }
...
function exportSelectedToCSV(selectedCheckboxes) {
    let csvContent = "\uFEFF"; 
    ...
    selectedCheckboxes.forEach(chk => {
        ...
        const addr = row.cells[2].querySelector('input')?.value || '';
        ...
        const visitDate = row.cells[7].querySelector('.visit-date-val')?.value || '';
        const serv = row.cells[8].querySelector('input')?.value || '';
        const val = row.cells[9].querySelector('input')?.value || '0';
        const status = row.cells[11].querySelector('select')?.value || '';
        const owner = row.cells[13].querySelector('input')?.value || '';

        const rowData = [comp, addr, mgr, mob, email, record, visitDate, serv, val, status, owner]
...
function printSelectedRows(selectedCheckboxes) {
    const selectedIds = new Set(Array.from(selectedCheckboxes).map(chk => chk.closest('tr')?.id).filter(Boolean));
    ...
    <head>
        <meta charset="UTF-8">
        <title>طباعة الزيارات المختارة</title>
        <style>
            body { font-family: 'Cairo', sans-serif; padding: 20px; direction: rtl; }
            h2 { text-align: center; color: #203764; margin-bottom: 20px; }
            table { width: 100%; border-collapse: collapse; margin-top: 10px; }
            th, td { border: 1px solid #cbd5e1; padding: 8px; text-align: center; font-size: 11px; }
            th { background-color: #203764; color: white; }
            tr:nth-child(even) { background-color: #f8fafc; }
        </style>
    </head>
    <body>
        <h2>تقرير الزيارات المحددة</h2>
        <table>
            <thead>
                <tr>
                    <th>الشركة</th><th>العنوان</th><th>المسؤول</th><th>رقم التواصل</th>
                    <th>تاريخ الزيارة</th><th>الخدمة</th><th>القيمة</th><th>الحالة</th><th>المالك</th>
                </tr>
            </thead>
            <tbody>`;
    ...
    selectedIds.forEach(id => {
        ...
        if (row) {
            ...
            <tr>
                ...
                <td>${escapeHTML(mgr)}</td>
                <td>${escapeHTML(mob)}</td>
...
function renderCalendarDays(dateObj) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (targetSelectedDate && day === targetSelectedDate.getDate() && month === targetSelectedDate.getMonth() && year === targetSelectedDate.getFullYear()) {
...
function selectDate(day, month, year) {
    ...
    if (row) {
        const visibleInput = row.cells[7].querySelector('input.readonly-input');
        const hiddenInput = row.querySelector('.visit-date-val');
        
        if (visibleInput) visibleInput.value = formattedDate;
        if (hiddenInput) hiddenInput.value = formattedDate;

        updateEditDateField(row);
        debouncedSaveSingleRow(targetRowForDatePicker);
    }
...
// ==========================================
// تصدير الدوال للكائن العام window لتعمل مع أحداث HTML مباشرة
// ==========================================
