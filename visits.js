...
function applyStatusColor(selectEl) { 
    ...
    if (val === 'تأهيل لفرصة') {
        ...
        selectEl.classList.add('status-yellow-fff'); // FFFF00
    } else if (val === 'عرض سعر') {
        selectEl.classList.add('status-yellow-ffc'); // FFC000
    } else if (val === 'غير مهتم') { 
        selectEl.classList.add('status-gray-a5'); // A5A5A5
        if (mainRow) mainRow.classList.add('row-shrink'); 
    } else if (val === 'فقدان') { 
        selectEl.classList.add('status-red-c00'); // C00000
        if (mainRow) mainRow.classList.add('row-shrink'); 
    } 
}
...
function updateEditDateField(row) { 
    ...
    // الاحتفاظ بالوقت في القيمة المخفية للبيانات السحابية إن دعت الحاجة لترتيبها
    if (hiddenInput) hiddenInput.value = `${dateFormatted} ${time24}`; 
    
    // إزالة الوقت والاكتفاء بالتاريخ فقط في واجهة المستخدم (الجدول الرئيسي)
...
function parseEditDateHTML(fullDateTime) { 
    ...
    // أخذ الجزء الخاص بالتاريخ فقط (العنصر الأول قبل المسافة)
