...
async function migrateToOpportunities(rowId) {
    ...
    try {
        ...
        Swal.fire('تم بنجاح', 'تم ترحيل البيانات إلى الفرص البيعية بنجاح', 'success');
        addToActivityLog('ترحيل', 'زيارة', 'فرصة بيعية', compVal);

    } catch (error) {
        console.error("خطأ في الترحيل للفرص:", error);
        Swal.fire('خطأ', 'حدث مشكلة أثناء محاولة الترحيل للفرص البيعية', 'error');
    }
}
...
async function handleStatusChange(selectEl, rowId) {
    ...
    if (newVal === 'تأهيل لفرصة' && oldVal !== 'تأهيل لفرصة') {
        Swal.fire({
            ...
            text: 'هل تريد ترحيل بيانات هذه الزيارة إلى صفحة الفرص البيعية؟',
            icon: 'question',
            ...
            if (result.isConfirmed) {
                ...
                // يمكنك إعادة الحالة السابقة إذا تم الإلغاء بإلغاء التهميش أدناه
