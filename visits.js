...
async function migrateToOpportunities(rowId) {
    ...
    if (subRow) {
        subRow.querySelectorAll('.product-body tr').forEach(pRow => {
            const inputs = pRow.querySelectorAll('input, select');
            if (inputs.length >= 5 && inputs[0].value) { // التأكد من وجود منتج مختار
                products.push({ 
                    type: inputs[0].value, 
                    desc: inputs[1].value, 
                    qty: inputs[2].value, 
    ...
    const oppData = {
        ...
        owner: owner,
        products: products,
        status: 'فرصة جديدة', // حالة افتراضية للترحيل
    ...
    try {
        ...
        // حفظ محلي ليتوافق مع نظام البحث الشامل (Local Search) الخاص بك
...
// تعديل الدالة لتدعم رسالة التأكيد والترحيل
async function handleStatusChange(selectEl, rowId) {
    ...
    // إذا تم اختيار تأهيل لفرصة، ولم تكن هذه هي الحالة السابقة
    if (newVal === 'تأهيل لفرصة' && oldVal !== 'تأهيل لفرصة') {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // يمكنك إعادة الحالة السابقة إذا تم الإلغاء
...
// لتتمكن عناصر HTML من الوصول إليها
// ==========================================
