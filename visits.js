...
function listenToVisits() {
    ...
    onSnapshot(visitsRef, (snapshot) => {
        ...
        // فرز الزيارات تنازلياً لإنشاء الفواصل الزمنية
...
function renderRow(v = {}) {
    ...
    mainRow.innerHTML = `
        ...
        <td><input type="text" class="excel-input" value="${v.comp || ''}" title="${v.comp || ''}" data-old="${v.comp || ''}" onfocus="this.dataset.old=this.value" oninput="this.title=this.value" onkeyup="updateEditDateField(this.closest('tr'));" onchange="saveSingleRow('${rowId}');" onblur="addToActivityLog('الشركة', this.dataset.old, this.value, this.value); this.dataset.old=this.value;"></td>
        ...
        <td><input type="text" dir="ltr" class="excel-input readonly-input" value="${visitDate}" style="color:var(--text-muted); font-weight:700;" readonly><input type="hidden" class="visit-date-val opp-date-val" value="${visitDate}"></td>
        <td><input type="text" class="excel-input cur-serv-val" value="${v.curServ || ''}" title="${v.curServ || ''}" data-old="${v.curServ || ''}" onfocus="this.dataset.old=this.value" oninput="this.title=this.value" onkeyup="updateEditDateField(this.closest('tr'));" onchange="saveSingleRow('${rowId}');" onblur="addToActivityLog('الخدمة', this.dataset.old, this.value, this.closest('tr').cells[1].querySelector('input').value); this.dataset.old=this.value;"></td>
...
async function handleStatusChange(selectEl, rowId) {
    ...
    if (newVal === 'تأهيل لفرصة' && oldVal !== 'تأهيل لفرصة') {
        ...
        if (confirm.isConfirmed) {
            ...
            try {
                await setDoc(doc(db, "opportunities", rowId), oppData);
...
function applyStatusColor(selectEl) { 
    ...
    selectEl.classList.remove('status-green', 'status-yellow-ffc', 'status-yellow-fff', 'status-red-ff0', 'status-red-c00'); 
    ...
    if (val === 'تأهيل لفرصة') {
        ...
        selectEl.classList.add('status-red-ff0'); 
...
function updateEditDateField(row) { if (!row) return; const dateFormatted = getTodayFormatted(); const time24 = getTimeFormatted(); const hiddenInput = row.querySelector('.edit-date-val'); if (hiddenInput) hiddenInput.value = `${dateFormatted} ${time24}`; const editMains = row.querySelector('.edit-date-container-main'); if(editMains) editMains.innerHTML = `<span class="edit-date-d" dir="ltr">${dateFormatted}</span><span class="edit-date-t" dir="ltr">${time24}</span>`; const subRow = document.getElementById('sub-' + row.id); if (subRow) { const subContainer = subRow.querySelector('.edit-date-container-sub'); if (subContainer) subContainer.innerHTML = `<span class="edit-date-d" dir="ltr">${dateFormatted}</span><span class="edit-date-t" dir="ltr">${time24}</span>`; } }
function parseEditDateHTML(fullDateTime) { if (!fullDateTime || !fullDateTime.includes(' ')) return `<span class="edit-date-d" dir="ltr">${formatAsDDMMYYYY(fullDateTime) || ''}</span><span class="edit-date-t"></span>`; const parts = fullDateTime.split(' '); return `<span class="edit-date-d" dir="ltr">${formatAsDDMMYYYY(parts[0])}</span><span class="edit-date-t" dir="ltr">${parts[1]}</span>`; }
    ...
    if (action === 'حذف') {
        ...
        if (result.isConfirmed) {
            for (let chk of selected) {
                ...
                if (row && row.id) await deleteDoc(doc(db, "visits", row.id));
            }
...
function addToActivityLog(fieldName, oldVal, newVal, companyName) { 
    ...
    const fullLogHTML = `<div class="log-entry"><span class="log-badge-user"><i class="fas fa-user-circle"></i> المستخدم</span><span class="log-divider">|</span><span class="log-timestamp" dir="ltr"><i class="fas fa-clock"></i> ${dd}-${mm}-${yyyy} ${timeStr} ${dayName}</span><span class="log-divider">|</span><span class="log-action">${actionText}</span></div>`; 
