...
function updateRowDOM(v) {
    ...
    if (statusSelect && document.activeElement !== statusSelect) {
        ...
        if (typeof applyStatusColor === 'function') applyStatusColor(statusSelect);
    }
...
function renderRow(v = {}) {
    ...
    mainRow.innerHTML = `
        ...
        <td class="col-status">
            <select class="excel-input status-select" onchange="if(typeof applyStatusColor === 'function') applyStatusColor(this)">
                <option value=""></option>
    ...
    inputs.forEach(input => {
        ...
        input.addEventListener('change', () => {
             if (typeof handleInputChange === 'function') handleInputChange(input, rowId);
        });
    });
...
// ==========================================
// الدوال المضافة لحل مشكلة توقف العرض
// ==========================================
function addProductRow(rowId, product = {}) {
    ...
    tr.innerHTML = `
        <td><input type="text" class="excel-input prod-name" value="${escapeHTML(name)}" placeholder="اسم الخدمة/المنتج" onchange="if(typeof handleInputChange === 'function') handleInputChange(this, '${rowId}')"></td>
...
function calculateMainVisitValue(rowId, triggerSave = true) {
    ...
    if (oppInput && oppInput.value !== totalValue.toString()) {
        ...
        if (triggerSave && typeof handleInputChange === 'function') {
...
// ==========================================
// محرك البحث الشامل وإدارة النتائج (مضاف)
// ==========================================
document.addEventListener('DOMContentLoaded', () => {
    // تفعيل الوضع الليلي أولاً إذا لزم
    initDarkMode();
...
// تعريض الدوال الرئيسية لـ window إذا استُدعيت مباشرة من HTML أو لتجنب أخطاء النطاق المرجعي
