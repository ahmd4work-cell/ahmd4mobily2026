...
const saveTimeouts = {}; // تم التعديل: استخدام كائن للتعامل مع كل صف بشكل مستقل لمنع ضياع البيانات
let searchTimeout;
...
function listenToVisits() {
    ...
    onSnapshot(visitsRef, (snapshot) => {
        ...
        // تم التعديل: التقاط الأخطاء لعدم إيقاف السكربت عند انقطاع الاتصال
...
// تم التعديل: إصلاح مشكلة تداخل المُؤقت عند التعديل على أكثر من حقل بسرعة
...
// تم التعديل: توفير دوال الحذف لتجنب ReferenceError عند التفاعل مع أيقونة النافذة في HTML
function closeDeleteModal() {
    const modal = document.getElementById('deleteModal');
    if (modal) modal.style.display = 'none';
}
function confirmDelete() {
    // هذه الدالة بمثابة احتياط إذا تم الاستدعاء من النافذة مباشرة
    // نظام الحذف المجمع يعتمد حالياً على SweetAlert بشكل أأمن
    closeDeleteModal();
}
...
// تم التعديل: تصدير دوال النافذة المنبثقة لمنع الخطأ عند الضغط في الأماكن الفارغة
